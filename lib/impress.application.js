...
impress.application.mixin = (application) => {
  ...
  if (impress.mode === 'test') {
    loadSteps.push('loadTests');
    loadSteps.push('runTests');
  }

  application.createApplication = (callback) => {
    const fns = loadSteps.map(fn => (...args) => application[fn](...args));

    api.metasync.sequential(fns, () => {
  ...
  function apiWrap(source) {
    return USE_STRICT + '(connection => (' + source + '))';
  }

  function codeWrap(source) {
    const isObj = source[0] === ASCII_BRACE_OPENING;
    const code = isObj ? '(' + source + ')' : source;
    return USE_STRICT + code;
  }

  function testWrap(source) {
    return `${USE_STRICT}((test, done) => {${source}})`;
  }
  ...
  if (impress.mode === 'test') {
    application.loadTests = (callback) => {
      const relPath = '/test/';
      const absPath = api.path.join(application.dir, relPath);
      api.fs.readdir(absPath, (err, files) => {
        if (err) {
          application.log.error(impress.CANT_READ_DIR + absPath);
          callback(err);
        ...
        api.metasync.each(files, (fileName, cb) => {
          ...
          api.fs.stat(filePath, (err, stats) => {
            ...
            application.loadTest(fileName, filePath, cb);
          });
        }, callback);
      });
    };

    application.loadTest = (fileName, path, callback) => {
      const fileExt = api.common.fileExt(fileName);
      if (fileExt !== 'js') {
        callback();
        return;
      }
      const testName = api.common.removeExt(fileName);
      application.createTest(path, (err, test) => {
        if (err) {
          callback(err);
          return;
        }
        application.tests.set(testName, test);
        callback();
      });
    };

    application.createTest = (fileName, callback) => {
      api.fs.readFile(fileName, (err, source) => {
        if (err) {
          application.log.error(impress.CANT_READ_FILE + fileName);
          callback(err);
          return;
        ...
        const code = testWrap(source);
        const script = new api.vm.Script(code, options);
        const test = script.runInContext(application.sandbox, options);
        callback(null, test);
      });
    };

    application.runTests = (callback) => {
      const runTest = ([testName, test], callback) => {
        api.tap.test(`Testing ${testName}`, (tap) => {
          test(tap, () => {
            tap.done();
            callback();
          });
      ...
      api.metasync.series(Array.from(application.tests.entries()), runTest,
        () => {
          api.tap.done();
          callback();
        }
