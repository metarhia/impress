...
const getHash = (password, callback) =>
  api.argon2.hash(password, { type: api.argon2.argon2id }).then(
    hash => {
      callback(null, hash);
    },
    error => {
...
const verifyHash = (hash, password, callback) =>
  api.argon2.verify(hash, password).then(matches => {
    if (!matches) {
...
class Security {
  ...
  createSession(session, callback) {
    ...
    this.gs.create(session, err => {
      if (err) {
        this.application.log.error(err);
