...
if (api.mysql) {
  ...
  api.db.mysql.open = function (database, callback) {
    ...
    if (database.opt && database.opt.connectionLimit && database.opt.connectionLimit > 1) {
      var pool = api.mysql.createPool(database.opt);

      pool.slowTime = database.slowTime;
      api.db.mysql.upgrade(pool);
      if (api.db.mysql.introspection) api.db.mysql.introspection(pool);

      pool.on('query', function (err, res, fields, query) {
        if (err) application.log.error('MySQL Error[' + err.errno + ']: ' + err.code + '\t' + query.sql);
        application.log.debug(query.sql);
      });

      pool.on('slow', function (err, res, fields, query, executionTime) {
      ...
      pool.on('enqueue', function () {
        application.log.error('Waiting for available connection slot');
      });

      database.connection = pool;
    } else {
      database.retryCounter++;
      var connection = api.mysql.createConnection(database.url || database.opt);
