...
// Metadata for api.impress.escapeRegExp
//
  ...
  try {
    ...
    if (api.impress.startsWith(err.message, 'Cannot find module')) {
      if (api.cluster.isMaster) {
        var msg = 'Module "' + (npmName || moduleName) + '" can`t be loaded, you need to install it using npm';
...
api.impress.logException = function(err, application) {
  ...
  stack = (
    stack.replace(rxPath1, '').replace(rxPath2, '').replace(rxPath3, '').replace(rxPath4, '')
