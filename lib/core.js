...
process.on('multipleResolves', (type, promise, reason) => {
  ...
  if (type === 'resolve') {
    const ser = api.util.inspect(promise);
    impress.log.warn(`${msg}, promise: ${ser}`);
  } else {
    const sp = api.util.inspect(promise);
    const sr = impress.shortenStack(api.util.inspect(reason));
    impress.log.warn(`${msg}, promise: ${sp}, reason: ${sr}`);
  }
...
process.on('unhandledRejection', (reason, promise) => {
  const ser = api.util.inspect(promise);
  impress.log.warn(`unhandledRejection promise: ${ser}, reason: ${reason}`);
});

process.on('rejectionHandled', promise => {
  impress.log.warn(api.util.inspect(promise));
});
