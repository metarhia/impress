...
// Stateful Application pligin definition
//
impress.state.mixinApplication = function(application) {

  var state = {
    data: {},         // data structures for synchronization
    delta: {},        // delta to be synced at next syncronization
    back: {},         // data for rollback
    timer: null,      // timer object or null
    timeout: null,    // next timer event timestamp in milliseconds
    mode: 'lazy',     // syncronization mode: lazy, transaction
    started: false,   // transaction started or not
    changed: false,   // state changed locally and not synchronized
    subscribers: {},  // hash of arrays, key is path, value is array of function
    watchers: {}      // hash of arrays
  };
  ...
  function sendDelta() {
    ...
    if (state.changed) {
      ...
      state.delta = {};
      state.changed = false;
      application.events.sendGlobal('state', { delta: delta });
    }
  }
  ...
  //
  application.on('state', function(data) {
    //console.log('application.on.state in '+impress.nodeId);
