...
class Security {
  ...
  createDataStructures(callback) {
    ...
    this.gs.generateSchema(securitySchema, err => {
      if (err) impress.log.error(err);
      else impress.log.info('Database schema was generated');
      callback(err);
    });
  }

  dropDataStructures(callback) {
    // TODO: to be implemented and integrated with gs
    callback();
  }
    ...
    getHash(password, (error, hash) => {
      ...
      this.getUser(login, (err, user) => {
        ...
        this.gs.create(record, err => {
          if (!err) {
            application.startSession(client);
  ...
  startSession(client) {
    if (!client.session) {
      ...
      if (impress.config.sections.scale.cookie) {
        client.setCookie(impress.config.sections.scale.cookie, impress.nodeId);
      }
  ...
  destroySession(client) {
    ...
    if (session) {
      ...
      application.security.deleteSession(session.token);
      client.session = null;
    }
  }
