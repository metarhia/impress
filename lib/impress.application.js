...
impress.mixinApplication = function(application) {
  ...
  application.backend = impress.eventEmitter();
  application.frontend = impress.eventEmitter();
  application.nodeId = impress.nodeId;
  application.isMaster = api.cluster.isMaster;
  application.isWorker = api.cluster.isWorker;

  application.on('start', function(data) {
  ...
  // RPS Limits (request per second)
  // Arrays of structure { rps, time }
  //   rps - request counter
  //   sec - time in secconds
  //
  application.limit = {
    ips: [], // keyed by IP
    sid: []  // keyed by SID
  ...
  application.createSandbox = function(callback) {
    ...
    application.loadConfigFile('sandbox.js', function() {
      if (application.config.sandbox) {
        var moduleLink, moduleName, moduleNpm, moduleRecord;
        var globals = application.config.sandbox.global || impress.DEFAULT_SANDBOX;
        for (var i = 0, len = globals.length; i < len; i++) {
          moduleName = globals[i];
          if (moduleName === 'require') moduleLink = require;
        ...
        for (var j = 0, jlen = apis.length; j < jlen; j++) {
          moduleName = apis[j];
          moduleRecord = api.registry.find(moduleName);
          if (moduleRecord) {
            moduleNpm = moduleRecord.npm;
            moduleName = moduleRecord.name;
          } else {
            moduleNpm = moduleName;
            moduleName = api.impress.spinalToCamel(moduleName);
          }
          moduleLink = api[moduleName];
          if (!moduleLink) moduleLink = api.impress.require(moduleNpm);
