...
impress.cloud.call = function(app, name, data, callback) {
  if (impress.cloud.status === 'online') {
    ...
    var packet = socket.packet('call', app, name, data);
...
impress.cloud.event = function(app, name, data) {
  if (impress.cloud.status === 'online') {
    ...
    var packet = socket.packet('event', app, name, data);
...
impress.cloud.upgradeSocket = function(socket) {
  ...
  socket.packetId = 0;
  setTimeout(function() {
    var packet = socket.packet('handshake');
    socket.sendPacket(packet);
  }, 1000);
  ...
  //
  socket.packet = function(type, app, name, data, id, target, to) {
    ...
    return {
      ...
      app: app,
      name: name,
      target: target, // frontend, backend, user, channel
      to: to,
      data: data
    };
  };
  ...
  var methods = {
    ...
    call: function(packet) {
      ...
      var reply = socket.packet('callback', packet.app, undefined, { result: true }, packet.id);
    ...
    event: function(packet) {
      ...
      if (application) {
        ...
        if (!isTarget) impress.cloud.socket.broadcast(packet);
          ...
          if (target) target.emit(packet.name, packet.data);
        }
      }
    },
...
impress.cloud.mixinApplication = function(application) {
  ...
  application.frontend.on('*', function(eventName, data) {
    var isTarget = impress.cloud.role === 'client';
    console.dir({frontendEvent:arguments});
    //console.log('application.frontend.on event in '+impress.nodeId+' isTarget='+isTarget);
    if (!isTarget) impress.cloud.event(application.name, eventName, data);
    if (application.sse) application.sse.sendGlobal(eventName, data);
    //if (isTarget) application.frontend.emit(eventName, data);
  });

  application.backend.on('*', function() {
    console.dir({backendEvent:arguments});
  });
