...
impress.log.mixin = (application) => {
  ...
  application.log.init = () => {
      ...
      if (application === impress || applicationLog) {
        api.mkdirp(application.log.dir, (err) => {
          if (err) {
            ...
            let nextReopen = nextDate - now + DAY_MILLISECONDS;
    ...
    application.log.openFile = (fileType, callback) => {
      let perNode = impress.log.perNodeTypes.indexOf(fileType) !== -1;
      if (process.isMaster && perNode) callback();
      else {
        let date = api.common.nowDate();
        let fileName = application.log.dir + '/' + date + '-' + fileType;
        if (perNode) fileName += '-' + impress.nodeId;
        fileName += '.log';
        application.log.closeFile(fileType, () => {
          let fd = api.fs.createWriteStream(fileName, {
            flags: 'a', highWaterMark: writeBuffer
          });
          let timer = api.timers.setInterval(
            makeTimer(fileType),
            writeInterval
          );
          let file = { fd: fd, buf: '', lock: false, timer: timer };
    ...
    application.log.closeFile = (fileType, callback) => {
      let file = application.log.files[fileType];
      if (file) {
        let filePath = file.fd.path;
