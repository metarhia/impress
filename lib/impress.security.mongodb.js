(function(impress) {
		...
		rl.question("Delete stored sessions and users if any? [y/n]: ", function(answer) {
			...
			if (answer == 'y') {
				var steps = [
					...
					function(callback) {
						db.impress.groups.remove({}, function(err, collection) {
					...
					function(callback) {
						db.impress.users.createIndex( { login: 1 }, { unique: true }, callback);
					},
					function(callback) {
						db.impress.sessions.createIndex( { sid: 1 }, { unique: true }, callback);
					},
					function(callback) {
						db.impress.groups.createIndex( { name: 1 }, { unique: true }, callback);
					},
					function(callback) {
					...
					function(callback) {
						db.impress.groups.insert([ { name: "users" }, { name: "admins" }, { name: "employees" } ], callback);
					}
				];
	...
	impress.security.deletePersistentSession = function(sid, callback) {
		db.impress.sessions.remove({ sid: sid }, true, function(err) {
			if (callback) callback();
		});
	};
