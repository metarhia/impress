...
cd /usr/src
sudo wget https://nodejs.org/dist/v12.0.0/node-v12.0.0.tar.gz
sudo tar zxf node-v12.0.0.tar.gz
sudo rm -f ./node-v12.0.0.tar.gz
cd node-v12.0.0
sudo ./configure
sudo make
sudo make install
cd ~
sudo rm -rf /usr/src/node-v12.0.0
sudo mkdir /ias
cd /ias
