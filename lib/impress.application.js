...
//
//impress.application.dirs = ['www', 'static', 'config', 'tasks', 'init', 'setup', 'model', 'lib', 'files', 'templates'];
...
impress.application.mixin = function(application) {
  ...
  application.isMaster = process.isMaster;
  application.isWorker = process.isWorker;
  ...
  application.createSandbox = function(callback) {
    var sandbox = {
      module: {},
      api: {},
      callInContext: api.impress.callInContextMethod
    };
  ...
  application.preprocessConfig = function() {
    ...
    var server, serverName, serviceName, toType,
        from, to, count, cpus,
        i, ilen, srv, port,
    ...
    for (serverName in servers) {
      ...
      if (server.ports.length > 1) {
        ...
        if (toType === 'undefined') {
          to = server.ports[2];
          server.ports = api.common.range(from, to);
          ...
          server.ports = api.common.range(from, from + count - 1);
        }
      }
  ...
  //
  application.setupScriptChanged = function(placeDir, scriptName, doneFileName, callback) {
    ...
    application.createScript(placeDir + '/' + file, function(err, exports) {
      if (!err) {
        if (placeName === 'tasks') application.setTask(sectionName, exports);
          ...
          api.fs.writeFile(
            placeDir + '/' + sectionName + '.done', new Date().toISOString()
          );
        }
      }
      callback();
  ...
  application.startTask = function(taskName) {
    ...
    if (task && !task.active) {
      if (
        (process.isMaster && task.place === 'master') ||
        (process.isWorker && task.place === 'worker')
      ) {
        task.active = true;
      ...
      for (var iRoute = 0, len = routes.length; iRoute < len; iRoute++) {
        ...
        if (match) {
          return application.dispatchRoute(
            client, route, match, req, res, iRoute
          );
        }
      }
    }
