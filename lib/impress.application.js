...
impress.application.places = ['tasks', 'init', 'setup', 'model', 'lib'];
...
impress.application.mixin = function(application) {
    ...
    application.loadConfigFile('sandbox.js', function() {
      if (application.config.sandbox) {
        ...
        for (var j = 0, jlen = apis.length; j < jlen; j++) {
          ...
          moduleName = api.impress.spinalToCamel(moduleName);
  ...
  application.preloadDirectory = function(relPath, depth, callback) {
    ...
    api.fs.readdir(absPath, function(err, files) {
      ...
      if (err) {
        ...
        api.impress.each(files, function(fileName, cb) {
    ...
    api.fs.readdir(application.dir + '/config', function(err, files) {
      if (err) {
        ...
        api.async.filterSeries(files, function(file, cb) {
          ...
          api.impress.eachSeries(files, application.loadConfigFile, callback);
        });
      }
    });
  ...
  application.preprocessConfig = function() {
    ...
    if (config.scale && config.servers && config.scale.strategy === 'bundle') {
      var server, srv, serverName, servers = config.servers;
      config.servers = {};
      for (serverName in servers) {
        server = servers[serverName];
        if (server.bundle) {
          for (var i = 0, len = config.scale.workers; i < len; i++) {
            srv = api.impress.clone(server);
            srv.port += i;
            config.servers[serverName + srv.port] = srv;
          }
        } else config.servers[serverName] = server;
      }
    }
  ...
  application.loadPlaces = function(callback) {
    api.impress.each(impress.application.places, function(placeName, cb) {
      ...
      if (exists) {
        api.fs.readdir(placeDir, function(err, files) {
          if (err) {
            ...
            api.impress.each(files, function(file, cb) {
              if (api.impress.endsWith(file, '.js')) {
                if (placeName === 'setup') {
                  var sectionName = api.path.basename(file, '.js');
                  if (api.cluster.isWorker || api.impress.inArray(files, sectionName + '.done')) return cb();
                }
  ...
  application.handler = function(method, path, handler, meta) {
    var dirPath = '/server' + api.impress.addTrailingSlash(path),
