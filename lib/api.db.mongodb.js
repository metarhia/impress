...
function MongoCategory(collection) {
  ...
  this.updateById = function(object, callback) {
    var mID = new api.mongodb.ObjectID(object.id);
    delete object.id;
    collection.update({ _id: mID }, object, function(err, data) {
      if (err) return callback(err);
      callback(null);
    });
  };

  this.update = function(query, object, callback) {
    if (query.id) {
      var mID = new api.mongodb.ObjectID(query.id);
      delete query.id;
      query._id = mID;
    }
    collection.update(query, object, function(err) {
      callback(err);
    });
  };

  this.deleteById = function(objectId, callback) {
    var mID = new api.mongodb.ObjectID(objectId);
    collection.removeOne({ _id: mID }, function(err, data) {
      if (err) return callback(err);
      callback(null);
    });
  };
  ...
  this.delete = function(query, callback) {
    if (query.id) {
      var objectId = new api.mongodb.ObjectID(query.id);
      delete query.id;
      query._id = objectId;
    }
  ...
  // TODO: learn what magicBool does
  this.findOne = function(query, magicBool, callback) {
    if (query.id) {
      var mID = new api.mongodb.ObjectID(query.id);
      delete query.id;
      query._id = mID;
