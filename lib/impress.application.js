...
impress.systemTemplates = [ 'error', 'index', 'introspection' ];
...
impress.mixinApplication = function(application) {
  ...
  if (application !== impress) {
    ...
    for (var j = 0; j < impress.systemTemplates.length; j++) {
      templateName = impress.systemTemplates[j];
    ...
    if (fileExt === '.js') {
      if (!application.config[sectionName]) {
        application.createScript(configFile, function(err, exports) {
          ...
          if (api.cluster.isMaster && configDefinition[sectionName]) {
