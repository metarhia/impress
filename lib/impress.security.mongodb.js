...
//
//impress.security.mixinApplication = impress.security.mixinApplication.override(function(application) {
api.impress.override(impress.security, function mixinApplication(application) {
  ...
  application.security.createDataStructures = function(callback) {
    ...
    if (!db.mongodb) api.impress.log('  No MongoDB drivers found'.red.bold, callback);
    if (!db.mongodb.schema) api.impress.log('  No MongoDB schema plugin loaded'.red.bold, callback);
    else {
      var securitySchema = api.definition.require('impress.security.schema');
      if (!securitySchema) api.impress.log('  No Impress security database schema for MongoDB loaded'.red.bold, callback);
      else if (!application.security.hasDb()) api.impress.log('  Can not generate MongoDB database structure'.red.bold, callback);
      else {
