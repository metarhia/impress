...
impress.dir = api.process.cwd().replace(impress.BACKSLASH_REGEXP, '/');
...
//
api.process.on('uncaughtException', function(err) {
...
//
api.process.execArgv.forEach(function(val) {
...
if (impress.memoryLimit) {
  setInterval(function() {
    var mu = api.process.memoryUsage();
...
impress.server.start = function() {
  impress.loadPlugins();
  impress.mode = api.process.env.IMPRESS_MODE || '';
  impress.workerId = api.process.isMaster ? 0 : api.process.env.WORKER_ID;
  impress.workerType = api.process.env.WORKER_TYPE;
  impress.serverName = api.process.env.WORKER_SERVER_NAME;
    ...
    impress.loadConfig(function() {
      ...
      impress.log.open(function() {
        if (impress.workerType === 'long') {
          ...
          api.process.title = 'impress ' + impress.nodeId;
          impress.workerApplicationName = api.process.env.WORKER_APPNAME;
          impress.workerApplicationFile = api.process.env.WORKER_FILE;
          impress.workerApplicationClient = JSON.parse(api.process.env.WORKER_CLIENT);
          ...
          api.process.title = 'impress ' + (api.process.isMaster ? 'srv' : impress.nodeId);
        }
        impress.processMarker = (
          (api.process.isMaster ? 'Master':'Worker') +
          '(' + api.process.pid + '/' + impress.nodeId + ')'
        );
        if (api.process.isMaster && impress.config.scale && impress.config.scale.check) {
...
impress.server.restart = function() {
  if (api.process.isMaster) console.log('Restarting...'.green);
  if (impress.config) impress.stop(function() {
    if (api.process.isMaster) console.log('  Reloading server configuration');
...
impress.serverOnError = function(err) {
  if (['EADDRINUSE', 'EACCESS', 'EACCES'].indexOf(err.code) > -1) {
    ...
    if (api.process.isWorker) {
      api.process.send({ name: 'impress:exit', error: msg });
    } else {
...
impress.startIpc = function() {
  api.process.on('SIGINT', impress.shutdown);
  api.process.on('SIGTERM', impress.shutdown);

  if (api.process.isWorker) {
    // Receive events from master
    api.process.on('message', function(message, socket) {
...
impress.forkLongWorker = function(appName, workerFile, clientData) {
  ...
  if (application) {
    if (api.process.isMaster) {
      ...
      impress.nextWorkerId++;
      var worker = api.process.fork(env);
