...
class Application extends events.EventEmitter {
  ...
  createSandbox() {
    const { auth, config, server: { host, port, protocol } = {} } = this;
    ...
    const domain = {};
    const sandbox = {
      ...metavm.COMMON_CONTEXT, Error, console: this.console,
  ...
  async loadModule(fileName) {
    ...
    try {
      ...
      exports = iface = script.exports;
      if (typeof exports === 'function') exports = { method: exports };
      if (exports.method) iface = makePrivate(exports);
    } catch (e) {
      iface = null;
      exports = null;
    }
    ...
    for (let depth = 0; depth <= last; depth++) {
      ...
      if (next) {
        if (depth === MODULE && namespace === 'stop') {
          if (iface === null && level.stop) await this.execute(level.stop);
        }
      } else {
        next = depth === last ? iface.method : {};
