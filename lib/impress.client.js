...
Client.prototype.end = function(output, socket) {
  ...
  client.saveSession(function() {
    ...
    if (client.context && client.context.cache) {
      application.cache.pages[application.relative(client.realPath)] = {
...
Client.prototype.fileHandler = function(handler, inheritance, callback) {
  ...
  var fileName = handler + '.js',
      ...
      fileCache = application.cache.files[application.relative(filePath)];

  if (!inheritance) client.currentHandler = handler;
  if (fileCache) {
    if (fileCache !== impress.FILE_NOT_FOUND) client.runScript(handler, application.dir + fileCache, callback);
    else callback();
    ...
    if (exists) {
      ...
      if (!inheritance) application.cache.files[application.relative(fileOriginal)] = application.relative(filePath);
      ...
      if (client.execPath !== '/' && handler !== 'meta') {
        ...
        fileOriginal = client.pathDir + fileName;
        if (!inheritance) application.cache.files[application.relative(fileOriginal)] = impress.FILE_NOT_FOUND;
        application.watchCache(fileOriginal);
        callback();
      }
    }
...
Client.prototype.detectRealPath = function(callback) {
  ...
  var folderCache = application.cache.folders[application.relative(client.realPathDir)];
  if ((folderCache && folderCache !== impress.FILE_NOT_FOUND) || client.realPath === '/') callback();
    ...
    if (exists) {
      application.cache.folders[application.relative(client.realPath)] = impress.FILE_EXISTS;
      callback();
    } else {
...
Client.prototype.template = function(data, file, cursor, callback) {
  ...
  api.async.detectSeries(files, function(item, callback) {
    ...
    var fileOriginal;
    if (fileCache) {
      if (fileCache !== impress.FILE_NOT_FOUND) client.include(data, application.dir + '/' + fileCache, cursor, callback);
      ...
      application.cache.files[application.relative(fileOriginal)] = application.relative(filePath);
      application.watchCache(fileOriginal);
    } else {
      ...
      if ((client.execPath !== '/') && (client.execPath !== '.')) {
        ...
        fileOriginal = client.execPathDir + fileName;
        application.cache.files[application.relative(fileOriginal)] = impress.FILE_NOT_FOUND;
        application.watchCache(fileOriginal);
...
Client.prototype.include = function(data, filePath, cursor, callback) {
  ...
  var cache = application ? application.cache.templates[application.relative(filePath)] : null;
  if (cache) {
    ...
    api.fs.readFile(filePath, 'utf8', function(err, tpl) {
      if (err) callback(impress.TPL_NOT_FOUND + filePath + '!3');
        ...
        if (application) application.cache.templates[application.relative(filePath)] = tpl;
...
Client.prototype.static = function(callback) {
  ...
  if (client.path === '/') callback();
    ...
    var filePath = api.impress.stripTrailingSlash(application.staticDir + client.path),
    ...
    if (buffer) {
      ...
      if (err) {
        application.cache.static[application.relative(filePath)] = impress.FILE_NOT_FOUND;
        application.watchCache(filePath);
