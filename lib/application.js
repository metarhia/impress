...
class Application extends events.EventEmitter {
  ...
  async loadModule(fileName) {
    ...
    let exp = script ? script(EMPTY_CONTEXT) : null;
    let container = {};
    if (typeof exp === 'function') {
      const method = exp.bind(container);
      exp = method;
      ...
      for (const methodName of methods) {
        const fn = container[methodName];
        if (typeof fn === 'function') {
          const method = fn.bind(container);
          exp[methodName] = method;
          container[methodName] = method;
        }
      }
      if (exp.method) exp = exp.method;
    }
