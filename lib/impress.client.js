...
var Client = function(application, req, res) {
  ...
  this.res.setHeader('Content-Type', impress.mimeTypes[this.typeExt]);
  this.res.setHeader('Cache-Control', 'public');
};
...
Client.prototype.destroySession = function() {
  if (this.application && this.session) {
    ...
    this.deleteCookie(impress.config.cluster.cookie, this.application.config.sessions.domain);
