...
impress.application.mixin = function(application) {
  ...
  application.prepareScript = function(fileName, code) {
    ...
    try {
      ...
      scriptName = fileName.replace(application.isImpress ? impress.dir : impress.applicationsDir, '');
        ...
        api.common.each(files, function(fileName, cb) {
          ...
          api.fs.stat(filePath, function(err, stats) {
            if (!err && stats.isFile() && fileExt === 'js') {
              application.createScript(filePath, function(err, ex) {
                apiInterface[methodName] = ex;
                cb();
              });
            } else cb();
          });
