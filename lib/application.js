...
const STATUS = Symbol('NamespaceStatus');
const LOADING = 1;
const READY = 2;
...
class Application extends events.EventEmitter {
  ...
  createSandbox() {
    ...
    const sandbox = {
      console: this.console, Buffer, application, node, npm,
  ...
  async removeModule(namespaces) {
    let level = this.sandbox;
    for (let i = 0; i < namespaces.length; i++) {
      const namespace = namespaces[i];
      const next = level[namespace];
      if (!next || !next[STATUS]) return;
      if (i === namespaces.length - 1) {
        if (i === 1 && next.stop) await next.stop();
        for (const key of Object.keys(next)) {
          const value = next[key];
          if (value[STATUS]) return;
        }
        delete level[namespace];
        return;
      }
      level = next;
    }
  ...
  async loadModule(fileName) {
    ...
    if (!script) {
      await this.removeModule(namespaces);
      return;
    }
    ...
    for (const key of Object.keys(level)) {
      if (typeof level[key] === 'function') delete level[key];
    }
    Object.assign(level, exp);
    const last = namespaces.length - 1;
    if (level[STATUS] === LOADING && last === 1 && level.start) {
      await level.start();
    }
    level[STATUS] = READY;
  }
