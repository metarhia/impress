...
(function(impress) {
		...
		application.createScript = function(key, fileName, callback) {
			...
			if (exports) callback(null, key, exports);
				...
				if (!err) {
					...
					try {
						var script = impress.vm.createScript(code, scriptName);
	...
	// Applicaction mixins: hash and adding method
	//
	impress.applicationMixins = {};
	//
	impress.addApplicationMixin = function (pluginName) {
		var plugin = impress[pluginName];
		if (plugin && plugin.applicationMixin) impress.applicationMixins[pluginName] = plugin.applicationMixin;
	};

	// Client mixins: hash and adding method
	//
	impress.clientMixins = {};
	//
	impress.addClientMixin = function (pluginName) {
		var plugin = impress[pluginName];
		if (plugin && plugin.clientMixin) impress.clientMixins[pluginName] = plugin.clientMixin;
	};
