'use strict';

impress.systemTemplates = {};

const meta =
  '<meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0, user-scalable=no" />';
const refresh =
  '#refresh {margin:0 0 0 30px;padding:0 0 0 22px;background:url(data:image/svg+xml;base64,) 0 0 no-repeat;background-size:contain}';
const home =
  '#home {padding:0 0 0 22px;background:url(data:image/svg+xml;base64,) 0 0 no-repeat;background-size:contain}';
const logo =
  'data:image/svg+xml;base64,';

const html = ({ head, body }) =>
  `<!DOCTYPE html><html><head>${meta}${head}</head><body>${body}</body></html>`;

const script = `<script type='text/javascript'>
window.onload = () => {
  const table = document.getElementById("files");
  const rows = table.getElementsByTagName('tr');
  for (const row of rows) {
    row.onclick = function() {
      const a = this.getElementsByTagName('a')[0];
      document.location.href = a.href;
      return false;
    };
  }
  let curRow = null;
  let newRow = null;
  window.onkeydown = function(event) {
    const key = event.keyCode;
    if (key === 13 && curRow) {
      curRow.onclick.apply(curRow);
    } else if (curRow === null && [38, 40].includes(key)) {
      newRow = rows[1];
    } else if (curRow) {
      if (key === 38 && curRow.previousSibling && curRow.previousSibling.previousSibling) {
        newRow = curRow.previousSibling;
      } else if (key === 40 && curRow.nextSibling) {
        newRow = curRow.nextSibling;
      }
    }
    if (curRow) curRow.className = '';
    if (newRow) {
      newRow.className = 'selected';
      curRow = newRow;
      curRow.scrollIntoView(false);
    }
    return ![13, 38, 40].includes(key);
  };
};
</script>`;

impress.systemTemplates.error = (title, message) =>
  html({
    head: `<title>${title}</title>
<style>
body {background:#0079a1;font-family:Verdana, sans-serif;font-size:12pt}
body,a {color:#fff}
img {float:left;margin:0 10px 0 0}
div {margin:6px}
#title {font-size:16pt}
${refresh}
${home}
</style>`,
    body: `<a href="/"><img width="55" src="${logo}" /></a>
<div id="title">${title}</div>
<div id="msg">${message} <a href="" id="refresh">refresh</a>
<a href="/" id="home">home</a></div>`
  });

const css = `body {background:#0079a1;font-family:Verdana, sans-serif;font-size:10pt}
body,a {color:#fff;overflow-x: hidden;white-space: nowrap}
img {float:left;margin:0 10px 0 0}
div {margin:6px}
th {text-align:left}
td, th {padding:2px 15px 2px 2px;font-weight:normal;vertical-align:top}
tr:nth-child(even) {background: inherit}
tr:nth-child(odd) {background: #007097}
tr:hover, tr.selected {background:#78cef9;color:#000;cursor:pointer}
tr:first-child {background:#fff;color:#000;cursor:default}
a {color:inherit;text-decoration:none}
#title {font-size:16pt}
#msg {font-size:12pt}
#files {border-spacing:0;margin-top: 15px}
.size {text-align:right}`;

impress.systemTemplates.index = (title, path, files, dirs) => html({
  head: `<title>${title}: ${path}</title>
<style>
${css}
${home}
</style>
${script}`,
  body: `<a href="/"><img width="55" src="${logo}" /></a>
<div id="title">${title}</div>
<div id="msg"><a href="/" id="home"></a>${dirs}</div>
<table id="files">
<tr><th>Name</th><th class="size">Size</th><th>Modify time</th></tr>
${files}
</table>`
});

impress.systemTemplates.introspection = (title, path, files, dirs) =>
  html({
    head: `<title>${title}: ${path}</title>
<style>
${css}
${home}
</style>
${script}`,
    body: `<a href="/"><img width="55" src="${logo}" /></a>
<div id="title">${title}</div>
<div id="msg"><a href="/" id="home"></a>${dirs}</div>
<table id="files">
<tr><th>Call</th><th>Method</th><th>Modify time</th></tr>
${files}
</table>`
  });

impress.systemTemplates.file = (name, path, method, mtime) =>
  `<tr><td><a href="${path}">${name}</a></td><td>${method}</td><td>${mtime}</td></tr>`;
