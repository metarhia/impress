...
var initContext = function(context) {
  ...
  context.extend = function(obj, ext) {
    ...
    var property;
    for (property in ext) obj[property] = ext[property];
    return obj;
  };
  ...
  context.duration = function(s) {
    ...
    var result = 0, unit, match, key;
    if (typeof(s) === 'string') for (key in units) {
      unit = units[key];
  ...
  context.generateKey = function(length, possible) {
    var i, key = '';
    for (i = 0; i < length; i++) key += possible.charAt(random(0, possible.length - 1));
    return key;
  };
