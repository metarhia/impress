(function(impress) {
	...
	//
	impress.preprocessConfiguration = function() {
		// Prepare routes
	...
	impress.init = function(callback) {
		...
		if (impress.config.smtp.enabled) impress.smtp = impress.nodemailer.createTransport("SMTP",impress.config.smtp.options);

		impress.openDatabases(callback);

		// Config initialization
		impress.preprocessConfiguration();
	...
	// Final shutdown
	//
	impress.shutdown = function(callback) {
	...
	impress.file = function(req, res, file, callback) {
		...
		if (fileExecute) {
			if (fileExecute != impress.fileNotFound) impress.execute(req, res, fileExecute, callback);
			else impress.error(req, res, 404); // 'No handler found'
	...
	impress.template = function(req, res, data, file, cursor, callback) { // callback(tpl)
		...
		if (fileInclude) {
			...
			else callback(impress.templateNotFound+'(1)'+file);
		} else impress.fs.exists(filePath, function(exists) {
			if (exists) {
				...
				if (fileInclude) {
					if (fileInclude != impress.fileNotFound) impress.include(req, res, data, fileInclude, cursor, callback);
					else callback(impress.templateNotFound+'(2)'+file);
				} else impress.fs.exists(filePath, function(exists) {
					if (exists) {
						impress.include(req, res, data, filePath, cursor, callback);
						var fileOriginal = req.impress.hostDir+lastSlash(req.impress.url)+fileName;
						...
						if ((req.impress.path != '/') && (req.impress.path != '.')) {
							...
							impress.cache.files[fileOriginal] = impress.fileNotFound;
							impress.watchCache(fileOriginal);
							callback(impress.templateNotFound+'(3)'+file);
						}
					}
				});
			}
	...
	impress.include = function(req, res, data, filePath, cursor, callback) { // callback(tpl)
		...
		if (cache) {
			...
			impress.fs.readFile(filePath, 'utf8', function(err, tpl) {
				if (err) callback(impress.templateNotFound+'(4)'+filePath);
				else {
				...
				if (item.type == 'plain') {
					...
					impress.template(req, res, data, item.name, cursorNew, function(tpl) {
						if (tpl == impress.fileIsEmpty) callback();
							...
							result += tpl || impress.templateNotFound+'(5)'+item.name;
							//if (!tpl) throw new Error('ERROR5');
							callback();
						}
					});
				}
