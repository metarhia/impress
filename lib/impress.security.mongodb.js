...
impress.security.mixinApplication = impress.security.mixinApplication.override(function(application) {
  ...
  this.inherited(application);

  // Override User class for this application
  //
  var User = function(dbUser) {
    this.constructor.super_.apply(this, arguments);
    dbUser.id = dbUser._id;
    delete dbUser._id;
    api.impress.extend(this, dbUser);
  };
  api.util.inherits(User, impress.security.User);
  application.User = User;
  application.User.prototype.application = application;
  ...
  application.security.deletePersistentSession = function(client, sid, callback) {
    if (application.security.hasDb()) {
      application.databases.security.sessions.remove({ sid: sid }, true, function(err) {
        if (callback) callback();
      });
    } else callback();
  };
