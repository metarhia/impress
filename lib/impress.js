...
impress.fork = (env, inspect) => {
  let modulePath = process.argv[1],
      ...
      argv = api.common.clone(process.execArgv);
  if (inspect) {
...
//
impress.loadPlugins = () => {
  let plugin, pluginName, pluginPath;
  for (let i = 0, len = impress.CORE_PLUGINS.length; i < len; i++) {
...
impress.compareHosts = () => {
  let i, j, k, ilen, jlen, klen, appName,
      config, hosts, cmp = [];
  for (appName in impress.applications) {
...
impress.detectAppDir = (appName, callback) => {
  ...
  let linkFile = dir + '/application.link';
...
impress.loadApplications = (callback) => {
  if (process.isMaster) callback();
    ...
    let server = impress.config.servers[impress.serverName];
...
impress.loadApplication = (appName, dir, callback) => {
  let application = new api.events.EventEmitter();
  ...
  application.createSandbox(() => {
    application.loadConfig(() => {
      ...
      application.log.open(() => {
        ...
        api.db.openApplicationDatabases(application, () => {
...
impress.server.load = () => {
  ...
  impress.loadApplications(() => {
    ...
    if (impress.workerApplicationName) {
      let application = impress.applications[impress.workerApplicationName];
...
impress.ipc = () => {
  ...
  if (process.isWorker) {
    process.on('message', (message) => {
      ...
      let application = impress.applications[message.appName];
...
//
impress.server.stop = () => {
  let servers = impress.config.servers;
  impress.cache.clear();
  if (servers) {
...
impress.logServerStarted = (server) => {
  ...
  if (server.name === 'master') {
    let instanceType = impress.config.scale.instance;
...
impress.loadCertificates = (server) => {
  if (server.key && server.cert) {
    let certDir = impress.dir + '/config/ssl/';
    return {
...
impress.serverOnError = (err) => {
  if (['EADDRINUSE', 'EACCESS', 'EACCES'].indexOf(err.code) > -1) {
    let msg = 'Can`t bind to host/port ' + err.address;
    if (process.isWorker) {
...
impress.forkWorker = (workerId, serverName, inspect) => {
  ...
  worker = impress.fork(env, inspect);
...
impress.killWorkers = () => {
  let worker;
  for (let workerId in impress.workers) {
...
impress.forkLongWorker = (appName, workerFile, clientData) => {
  let application = impress.applications[appName];
  if (application) {
    if (process.isMaster) {
      let env = {};
      let workerId = impress.nextWorkerId;
...
impress.listenWorker = (worker) => {
  worker.on('message', (message) => {
    ...
    if (application) {
      if (message.name === 'impress:forklongworker') {
        let longWorker = impress.forkLongWorker(
          message.appName,
          message.workerFile,
          message.clientData
        );
...
impress.dispatcher = (req, res) => {
  ...
  let application, appName,
      host = api.common.parseHost(req.headers.host);
  for (appName in impress.applications) {
    application = impress.applications[appName];
  ...
  let client = new impress.Client(impress, req, res);
  client.error(404);
};
