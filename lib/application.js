...
class Application extends events.EventEmitter {
  ...
  addModule(namespaces, exports, iface) {
    ...
    for (let depth = 0; depth <= last; depth++) {
      ...
      if (next && depth === MODULE && namespace === 'stop') {
  ...
  removeModule(fileName) {
    ...
    const namespaces = this.namespacesModule(fileName);
    this.addModule(namespaces, null, null);
  }
  ...
  startWatch() {
    ...
    this.watcher.on('delete', async (filePath) => {
      ...
      else this.removeModule(filePath);
      if (node.worker.threadId === 1) {
