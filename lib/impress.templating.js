...
Client.prototype.template = function(data, file, cursor, callback) {
  ...
  var fileName, filePath, fileCache, relPath,
      files = [];
  if (file.indexOf('.') > -1) {
    filePath = api.path.normalize(client.execPathDir + file).replace(impress.BACKSLASH_REGEXP, '/');
  ...
  api.common.detectAsync(files, function(item, cb) {
    ...
    if (fileCache) {
      ...
      if (client.execPath !== '/' && client.execPath !== '/www' && client.execPath !== '.') {
        ...
        client.template(data, file, cursor, callback);
         application.cache.watch('/www' + api.common.stripTrailingSlash(client.execPath));
      } else {
...
Client.prototype.render = function(data, tpl, cursor, callback) {
  ...
  if (tpl !== impress.FILE_IS_EMPTY) {
    var structure = client.parseTemplate(data, tpl, cursor),
        result = '';
    api.common.eachSeries(structure, function(item, cb) {
      var cursorNew;
      if (item.type === 'plain') {
