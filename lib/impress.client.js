...
//
impress.createApplicationClientClass = function(mixin) {
...
Client.prototype.accessLog = function() {
	if (this.application && this.application.log) this.application.log.access(
...
//
Client.prototype.slowLog = function() {
	if (this.application && this.application.log) {
		this.endTime = new Date().getTime();
		if (this.endTime-this.startTime >= this.slowTime) this.application.log.slow(
...
Client.prototype.proxy = function(host, port, url) {
	...
	req.on('error', function(e) {
		if (client.application && client.application.log) client.application.log.error(
		...
		if (sid) {
			if (impress.validateSID(application.config, sid)) {
				if (impress.security && application.sessions[sid]) {
					client.session = sid;
					client.logged = !!application.sessions[sid].userId;
...
Client.prototype.processing = function() {
	var client = this,
		application = client.application,
		dirPath = client.hostDir+lastSlash(client.path);
	...
	api.async.eachSeries(client.handlers, function(handler, callback) {
		...
		if (handler === 'access' || client.access.virtual) client.fileHandler(handler, callback);
		else api.fs.exists(dirPath, function(exists) {
			if (exists) client.fileHandler(handler, callback);
		...
		if (client.access.allowed) {
			if (client.ext === '' && client.access.intro) {
				...
				else if (api.cms) api.cms.processing(client, processingPage);
				else client.processingPage();
			} else if (client.typeExt === 'sse') {
				if (client.sse) client.sseConnect(client);
				else client.error(510);
			} else if (client.typeExt === 'ws') {
				if (client.websocket) client.websocket.finalize(client);
				else client.error(510);
...
Client.prototype.processingPage = function() {
	...
	this.template(data, 'html', '', function(tpl) {
		if (client.cachable && client.application) {
			var filePath = client.hostDir+client.path;
			client.application.cache.pages[filePath] = tpl;
		}
		client.end(tpl);
	});
};
...
Client.prototype.introspect = function() {
	...
	api.fs.stat(introPath, function(err, stats) {
		if (err) client.error(404);
			...
			if (stats.isDirectory()) {
				...
				api.fs.readdir(introPath, function(err, flist) {
					var cbCount = flist.length, cbIndex = 0;
					var cb = function() {
						files.sort(function(a, b) {
							var s1 = a.name, s2 = b.name;
							...
							(function() {
								...
								api.async.parallel({
									stats: function(callback) {
										api.fs.stat(filePath, function(err, stats) { callback(null, stats); });
									},
									get: function(callback) {
										api.fs.exists(filePath+'/get.js', function(exists) { callback(null, exists); });
									},
									post: function(callback) {
										api.fs.exists(filePath+'/post.js', function(exists) { callback(null, exists); });
									},
	...
	if (fileExecute) {
		...
		if (exists) {
			...
			if (client.path !== '/' && file !== 'meta') {
				client.path = api.path.dirname(client.path);
				client.fileHandler(file, callback);
				var path = client.path;
				path = client.hostDir+lastSlash(path);
	...
	api.async.detectSeries(files, function(item, callback) {
		...
		if (fileInclude) {
			...
			if ((client.path !== '/') && (client.path !== '.')) {
				...
				impress.watchCache(application, client.hostDir+lastSlash(client.path));
			} else {
