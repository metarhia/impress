...
impress.loadApplications = function(callback) {
  api.fs.readdir(impress.applicationsDir, function(err, apps) {
    if (err) {
    ...
    } else api.async.each(apps, function(appName, cb) {
      ...
      if (stats.isDirectory() && (!impress.workerApplicationName || impress.workerApplicationName === appName)) {
        ...
        impress.createApplicationClientClass(application, {});
        api.impress.extend(application, { name: appName, dir: dir });
...
impress.startServers = function() {
  ...
  for (var serverName in servers) {
    ...
    if (master) {
      ...
      else if (multiple) server.listener = { close: emptyness, on: emptyness, listen: emptyness };
