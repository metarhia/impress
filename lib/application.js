...
const CFG_CONTEXT = metavm.createContext({ process });
const CFG_OPTIONS = { mode: process.env.MODE, context: CFG_CONTEXT };
...
class Application extends events.EventEmitter {
  constructor(options) {
    ...
    this.console = options.logger ? options.logger.console : null;
    this.auth = null;
    this.watcher = null;
    return this.load();
  }
  ...
  async load() {
    ...
    const config = await new Config(configPath, CFG_OPTIONS);
    this.config = config;
    await this.openLogger();
    if (!config.application || !config.application.extends) return this;
    const { root, logger } = this;
    for (const folder of config.application.extends) {
      const dir = folder.startsWith(path.sep) ? root : path.join(root, folder);
      const application = await new Application({ root, path: dir, logger });
  ...
  async openLogger() {
    ...
    const home = this.root;
    const dir = path.join(home, 'log');
    const options = { ...this.config.log, path: dir, workerId: threadId, home };
    const logger = await new Logger(options);
    this.logger = logger;
...
if (parentPort) {
  parentPort.on('message', async ({ type, name, data, port }) => {
    ...
    const application = Application.main;
...
const mainApplication = async () => {
  if (Application.main) return Application.main;
  const root = process.cwd();
  const dir = path.join(root, 'application');
  Application.main = await new Application({ root, path: dir });
