...
// On receive data from IPC message
//
process.on('message', function(packet) {
  console.dir({fromMaster:packet});
...
impress.cloud.transport.upgradeSocket = function(socket) {
  ...
  socket.sendPacket = function(type, name, data, id) {
    ...
    var packet = impress.cloud.packet(type, name, data, id);
    socket.process.send(packet);
    return packet.id;
  };

  socket.process.on('message', function(packet) {
    console.dir({fromWorker:packet,in:impress.nodeId});
    //server.emit('packet', packet);
  });
