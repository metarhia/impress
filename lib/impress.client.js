...
var Client = function(application, req, res) {
  ...
  res.on('finish', function() {
    ...
    socket.setTimeout(server.keepAlive, function() {
      socket.destroy();
    });
  });
};
...
Client.prototype.dispatch = function() {
  ...
  client.parseCookies();
  var code = impress.firewall.check(client);
...
Client.prototype.block = function(msec) {
  ...
  if (client.sid) impress.firewall.sid.deny(client.sid);
};
...
Client.prototype.startSession = function() {
  ...
  if (application && !client.session) {
    ...
    client.user = new application.security.User();
    client.session = new application.security.Session({ sid: client.sid });
    ...
    if (sid) {
      ...
      if (session) client.setSession(sid, session);
        ...
        if (
          ...
          var cb = callback;
          callback = null;
          application.security.readSession(sid, function(err, session) {
            if (err) {
...
Client.prototype.processing = function() {
  ...
  if (cache && (client.startTime < cache.expireTime)) client.sendCache(cache);
    ...
    if (client.typeExt === 'sse') client.eventChannel = null;
      ...
      if (client.res.setHeader && !client.res.headersSent) {
    ...
    // Execute handlers
    api.metasync.series(handlers, function(handler, cb) {
      if (
        handler === 'access' ||
...
Client.prototype.basicAuth = function() {
  ...
  if (authBase64) {
    ...
    var auth = api.common.buffer(authBase64, 'base64');
...
Client.prototype.detectRealPath = function(callback) {
  ...
  if (
    ...
    api.fs.exists(client.realPathDir, function(exists) {
      if (exists) {
...
Client.prototype.signIn = function(login, password, callback) {
  ...
  application.security.signIn(login, password, function(err, user) {
    if (user) {
      client.startSession();
...
Client.prototype.signOut = function(callback) {
  ...
  if (client.session) {
    var login = client.session.login;
    if (login) {
      var user = application.users[login];
...
Client.prototype.signUp = function(login, password, callback) {
  ...
  application.security.signUp(login, password, function(err, user) {
    if (user) {
      client.startSession();
