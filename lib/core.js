...
impress.start = () => {
  //impress.isInitialized = false;
  //impress.ready = false;
  //impress.backend = api.common.emitter();
  //impress.frontend = api.common.emitter();
  impress.config = new impress.Config(impress);
  //impress.scheduler = new impress.Scheduler(impress);
  //impress.model = {}; // key is model name, value is object
  //impress.users = new Map(); // key is login, value is instance of User
  //impress.sessions = new Map(); // key is sid, value is instance of Session
  //impress.channels = {};
  impress.workers = {};
  impress.longWorkers = {};
  //impress.api = {}; // all APIs will be loaded here on start
  ...
  for (let j = 0; j < impress.TEMPLATES.length; j++) {
    ...
    const tplPath = impress.dir + tplFile;
