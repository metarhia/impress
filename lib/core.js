...
impress.stop = callback => {
  ...
  const list = Object.values(servers).filter(server => server.instance);
  api.metasync.each(list, (server, next) => {
    server.instance.close(next);
  }, () => {
    for (const application of Object.values(impress.applications)) {
