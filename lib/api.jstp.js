...
// Create JSTP Server
//
api.jstp.createServer = function() {
  var server = api.net.createServer(api.jstp.dispatcher);
  server.clients = [];
...
Connection.prototype.process = function(packets) {
  ...
  function sendCallback() {
    var error  = arguments[0],
        ...
        stack  = connection.application.jstpCallStack;
    ...
    stack.pop();
    connection.application.sandbox.connection = stack[stack.size - 1];
  ...
  var kinds = {
    ...
    handshake: function () {
      ...
      if (packet.ok) {
        ...
        if (application) {
          ...
          application.jstpCallStack = application.jstpCallStack || [];
    ...
    call: function () {
      if (connection.application) {
        ...
        connection.application.jstpCallStack.push(connection);
...
Connection.prototype.callback = function(packetId, error, result) {
  if (error) {
    var packet = this.packet('callback', null, 'error', error);
  } else {
...
function RemoteError(code, message) {
  ...
  this.code = code;
  this.message = message || RemoteError.defaultMessages[code];

  if (this.message) {
    this.jstpArray = [this.code, this.message];
  } else {
    this.message = code;
    this.jstpArray = [this.code];
  }
...
api.jstp.transport = {};
api.jstp.transport.tcp = {};
api.jstp.transport.tls = {};
api.jstp.transport.ws = {};
api.jstp.transport.wss = {};
