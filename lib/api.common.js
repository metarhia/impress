...
api.common.generateKey = function(length, possible) {
  ...
  var bitsCount = Math.ceil(Math.log2(Math.pow(base, length + 1) - 1));
  var bytes = api.crypto.randomBytes(Math.ceil(bitsCount / 8));
  var key = '';
  for (var i = 0; i < length; i++) {
    var index = api.common.longDivModBE(bytes, base);
    key += possible[index];
  }
  return key;
};
...
//
api.common.longDivModBE = function(buffer, divisor) {
  if (divisor === 0) {
    throw new Error('Division by zero');
  }

  var bytesCount = Buffer.byteLength(buffer);
  var remainder = 0;
...
//
api.common.sequence = function(seq, max) {
  var from = seq[0],
      to = seq[1],
      res = seq;
  if (Array.isArray(to)) {
