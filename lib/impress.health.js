...
function clearDataStructures() {
  ...
  for (workerId in api.cluster.workers) {
    nodeId = impress.workers[workerId].nodeId;
    historyNodes[nodeId] = [];
    statNodes[nodeId] = [];
  }
}
...
impress.health.init = function() {
  if (api.cluster.isMaster) {
    ...
    if (impress.config.cluster.strategy === 'single') {
      ...
      }, impress.config.cloud.health);
    } else {
      var workerId;
      for (workerId in impress.workers) {
        impress.workers[workerId].on('message', function(msg) {
...
impress.health.saveHistoryCluster = function(fileName) {
  ...
  var i, data, line, fieldName;
  for (i = 0; i < historyCluster.length; i++) {
    data = historyCluster[i];
    line = [];
    for (fieldName in data) line.push(data[fieldName]);
    fd.write(line.join(';')+'\n');
  }
  fd.end();
  clearDataStructures();
...
impress.health.saveHistoryNodes = function(fileName) {
  var fd = api.fs.createWriteStream(fileName, { flags: 'a' });
  // node;time;uptime;rss;heap;used;req;res
  var i, nodeId, node, data, line, fieldName;
  for (nodeId in historyNodes) {
  ...
  if (impress.isWin) {
    ...
    cmdExec(cmdWmic, function(data) {
      ...
      for (i = 0; i < data.length; i++) {
        size = +data[i].Size, free = +data[i].FreeSpace;
        result.push({
