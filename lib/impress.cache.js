...
impress.cache.mixin = (application) => {
  ...
  application.updateFileCache = (
    ...
    api.fs.access(filePath, (err) => {
      if (err) return application.clearDirectoryCache(filePath);
      const ext = api.common.fileExt(filePath);
      const relPath = application.relative(filePath);
      api.common.clearCacheStartsWith(cache.pages, relPath);
      cache.files.delete(relPath);
      if (cache.static.has(relPath)) {
        return application.compress(filePath, stats);
      }
  ...
  application.clearDirectoryCache = (
    ...
    const relPath = application.relative(filePath);
    cache.static.clearStartsWith(relPath);
    api.common.clearCacheStartsWith(cache.folders, relPath);
    api.common.clearCacheStartsWith(cache.pages, relPath);
    api.common.clearCacheStartsWith(cache.files, relPath, (used) => {
