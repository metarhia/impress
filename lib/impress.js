...
impress.startServers = function() {
  ...
  for (var n = 0, len = serverNames.length; n < len; n++) {
    ...
    if (server.instance) {
      ...
      if (serverName === 'cloud') msg += ' Cloud Controller';
      console.log('  ' + msg);
      ...
      //console.log('Go listen server: ' + server.name + ' ' + server.address+':'+server.port);
      if (server.address === '*') {
...
impress.startIpc = function() {
  ...
  if (process.isWorker) {
    ...
    process.on('beforeExit', function(code) {
      //console.dir({mmm1:code});
      process.send({ name:'impress:exit', code: code }); // Closed
      impress.log.server('Terminated worker');
    });
...
impress.forkWorker = function(workerId, serverName) {
  ...
  //console.log('fork '+workerId+' in '+serverName);
  worker.on('exit', function(code, signal) {
    //console.dir({code,signal,worker:worker.id,workerId,serverName});
    impress.stat.forkCount--;
