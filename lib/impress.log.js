...
impress.log.fileTypes = [
  'access', 'api', 'error', 'debug', 'slow', 'server', 'node', 'cloud', 'warning'
];

// Log types to be separated per process
impress.log.perNodeTypes = [
  'access', 'api', 'error', 'debug', 'slow'
];

var DAY_MILLISECONDS = api.common.duration('1d');
var SEMICOLON_REGEXP = /;/g;
...
impress.log.mixin = function(application) {
  ...
  application.log.init = function() {
      ...
      if (application === impress || applicationLog) {
        api.mkdirp(application.log.dir, function(err) {
          if (err) {
            ...
            var now = new Date(),
                nextDate = new Date();
            api.metasync.each(application.log.fileTypes, function(fileType, cb) {
    ...
    application.log.openFile = function(fileType, callback) {
      var perNode = impress.log.perNodeTypes.indexOf(fileType) !== -1;
      if (process.isMaster && perNode) callback();
      else {
        var date = api.common.nowDate();
        var fileName = application.log.dir + '/' + date + '-' + fileType;
        if (perNode) fileName += '-' + impress.nodeId;
        fileName += '.log';
        application.log.closeFile(fileType, function() {
          var fd = api.fs.createWriteStream(fileName, {
            flags: 'a', highWaterMark: writeBuffer
          });
          var timer = api.timers.setInterval(
            makeTimer(fileType),
            writeInterval
          );
          var file = { fd: fd, buf: '', lock: false, timer: timer };
