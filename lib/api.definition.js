...
api.definition.walkRecursive = (parent, data, definition, name, path, cb) => {
  ...
  if (typeof(tpl) === 'string') {
    ...
    let tplKeys = typeof(tpl) === 'object' ? Object.keys(tpl) : [];
    let dataKeys = (
    ...
    let index = tplKeys.indexOf('_other');
...
api.definition.parse = (tpl) => {
  ...
  let name = tpl.replace(definitionRx, '');
