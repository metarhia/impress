...
impress.log.mixin = (application) => {
  ...
  application.log.init = () => {
    ...
    const def = impress.config.log;
    const config = application.config.log;

    application.log.fileTypes = config.files || impress.log.fileTypes;
    const writeInterval = config.writeInterval || def.writeInterval;
    const writeBuffer = config.writeBuffer || def.writeBuffer;
    const keepDays = config.keepDays || def.keepDays;
