...
(function(impress) {
	...
	//
	impress.log.init = function(application) {
		...
		application.log.openFile = function(fileType, callback) {
			...
			application.log.closeFile(fileType, function() {
				 var file = {
					fd: impress.fs.createWriteStream(fileName, { flags:'a', highWaterMark:writeBuffer }),
					buf: "",
					lock: false,
					timer: setInterval(makeTimer(fileType), writeInterval)
				};
				application.log.files[fileType] = file;
				file.fd.on('open',  function(fd)   { if (callback) callback(); });
				file.fd.on('error', function (err) { if (callback) callback(); });
			});
		};
		...
		application.log.deleteOldFiles = function() {
			impress.fs.readdir(application.log.dir, function(err, files) {
				if (!err) {
					var now = new Date();
					...
					for (var i in files) {
						...
						if (fileAge>1 && fileAge>keepDays) {
							impress.fs.unlink(application.log.dir+'/'+files[i], function(err) {});
						}
					}
				}
			});
