...
impress.loadPlugins = function() {
  ...
  for (var i = 0, len = impress.CORE_PLUGINS.length; i < len; i++) {
    ...
    console.log('  load: '+pluginName+' plugin: '+typeof(plugin)+' mixin: '+(plugin ? typeof(plugin.mixin) : '-'));
    if (pluginName ==='log' || pluginName ==='application' || pluginName ==='cache') plugin.mixin(impress);
    //if (plugin && plugin.mixin) plugin.mixin(impress);
  }
};
...
impress.mixinPlugins = function(application) {
  ...
  for (var i = 0, len = impress.CORE_PLUGINS.length; i < len; i++) {
    ...
    console.log('  mix: '+pluginName+' plugin: '+typeof(plugin)+' mixin: '+(plugin ? typeof(plugin.mixin) : '-'));
    if (plugin && plugin.mixin) plugin.mixin(application);
  }
};
...
impress.loadApplications = function(callback) {
  if (impress.workerApplicationName) {
    ...
    if (err) {
      ...
      api.impress.each(apps, function(appName, cb) {
...
impress.server.stop = function(callback) {
  ...
  impress.cache.clear();
  if (servers) {
    api.impress.each(Object.keys(servers), function(serverName, cb) {
...
impress.startServers = function() {
  ...
  for (var n = 0, len = serverNames.length; n < len; n++) {
    ...
    if (master) {
      ...
      else if (cluster) server.listener = { close: api.impress.emptyness, on: api.impress.emptyness, listen: api.impress.emptyness };
...
impress.startIpc = function() {
  ...
  if (api.cluster.isMaster && isFirstStart) {
    if (impress.config.scale.strategy === 'cluster' || impress.config.scale.strategy === 'sticky') {
...
impress.balancer = function(socket /* res */) {
  ...
  else if (impress.config.scale.strategy === 'cluster') ip = ~~(Math.random() * count);
