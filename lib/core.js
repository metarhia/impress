...
class Impress extends api.events.EventEmitter {
  constructor() {
    super();
    this.name = 'impress';
    this.isImpress = true;
    this.firstStart = true;
    this.isWorker = 'WORKER_SERVER_NAME' in process.env;
    this.isMaster = !this.isWorker;
    this.workerId = this.isMaster ? 0 : parseInt(process.env.WORKER_ID, 10);
    this.workerType = process.env.WORKER_TYPE;
    this.dir = process.cwd();
    this.isWin = process.platform.startsWith('win');
    this.applicationsDir = this.dir + '/applications';
    this.moduleDir = api.path.dirname(__dirname);
    this.mode = process.env.IMPRESS_MODE || '';
    this.serverName = process.env.WORKER_SERVER_NAME;
    this.serverProto = process.env.WORKER_SERVER_PROTO;
    this.nextWorkerId = 1;
    this.applications = {};
    this.stat = { fork: 0, event: 0, req: 0, res: 0 };
  }
}
  ...
  impress.createSandbox(() => {
    impress.loadConfig(() => {
      ...
      impress.processMarker = (
        (impress.isMaster ? 'Master' : 'Worker') +
