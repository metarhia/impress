...
if (api.mysql) {
  ...
  api.db.mysql.schema.generateScript = (databaseSchema, consoleOutput) => {
    ...
    if (validationResult.valid) {
      sqlScript = line + cmnt + databaseSchema.caption + crlf + cmnt +
        ...
        if (tableName !== 'caption' && tableName !== 'version') {
          ...
          for (const fieldName in table.fields) {
            ...
            if (
              ...
              reference = field.lookup ? field.lookup :
                field.master ? field.master : field.link;
              const referenceKey = reference.key || fieldName;
              referenceStatement = altr + tableName + ' ADD CONSTRAINT fk' +
                tableName + fieldName +
          ...
          if (table.comment) {
            sqlScript += altr + tableName + ' COMMENT = "' +
              table.comment + '";' + crlf + crlf;
          }
          if (primaryKey.length > 0) {
            sqlScript += altr + tableName + ' ADD CONSTRAINT pk' + tableName +
              ' PRIMARY KEY (' + primaryKey.join(', ') + ');' + crlf;
          }
          if (primaryKey.length === 1) {
            sqlScript += altr + tableName + ' CHANGE ' + primaryKey[0] + ' ' +
          ...
          if (references.indexes) {
            for (const indexName in references.indexes) {
              ...
              sqlScript += 'CREATE ' +
                (compositeIndex.unique ? 'UNIQUE ' : '') +
                'INDEX ' +
                (compositeIndex.unique ? 'ak' : 'idx') +
