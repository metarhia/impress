...
impress.security.hasDb = function(application) {
  ...
  if (!result) console.log('  No security database configured'.red);
  return result;
};
...
impress.security.createDataStructures = function(application, callback) {
  ...
  if (!db.mongodb) console.log('  No MongoDB drivers found'.red);
  else if (!db.mongodb.schema) console.log('  No MongoDB schema plugin loaded'.red);
  else {
    var securitySchema = require('../schemas/impress.security.schema.js');
    if (!securitySchema) console.log('  No Impress security database schema for MongoDB loaded'.red);
    else if (!impress.security.hasDb(application)) console.log('  Can not generate MongoDB database structure'.red);
    else {
      ...
      rl.question('Delete stored data and recreate default structures? [y/n]: ', function(answer) {
        rl.close();
        if (answer === 'y') db.mongodb.schema.generateSchema(securitySchema, true);
