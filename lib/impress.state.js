...
//
impress.state.mixin = function(application) {
  ...
  application.state.get = function(path) {
    return api.common.getByPath(state.data, path);
  };
  ...
  application.state.inc = function(path, value, delay) {
    ...
    state.changed = true;
    var data = api.common.getByPath(state.data, path);
  ...
  application.state.set = function(path, value, delay) {
    for (var key in state.delta) {
      if (api.common.startsWith(key, path)) {
  ...
  application.state.delete = function(path, delay) {
    for (var key in state.delta) if (api.common.startsWith(key, path)) delete state.delta[key];
    var item = { type: 'del' };
    state.delta[path] = item;
  ...
  application.state.change = function(path, value, remote) {
    ...
    for (var key in state.watchers) {
      if (api.common.startsWith(path, key)) {
