...
class Application extends api.events.EventEmitter {
  ...
  constructor(name, dir) {
    ...
    // SSE connections
    this.sse = new impress.ServerSentEvents(this);
    // JSTP connections
  ...
  dispatch(req, res) {
    ...
    const routes = this.config.sections.routes;
    if (routes) {
      for (let i = 0; i < routes.length; i++) {
        const route = routes[i];
        const match = req.url.match(route.urlRx);
        if (match) {
          this.dispatchRoute(client, route, match, i);
          return client;
        }
      }
    }
  ...
  dispatchRoute(client, route, match, iRoute) {
  ...
  shutdownLongWorkers() {
    for (const workerId in this.longWorkers) {
      this.longWorkers[workerId].kill();
    }
  }
