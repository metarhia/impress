...
impress.log.mixin = function(application) {
  ...
  application.log.init = function() {
    ...
    if (!writeInterval) {
      writeInterval = api.impress.getByPath(impress.config, 'log.writeInterval');
    }

    var writeBuffer = api.impress.getByPath(application.config, 'log.writeBuffer');
    if (!writeBuffer) {
      writeBuffer = api.impress.getByPath(impress.config, 'log.writeBuffer');
    }

    var keepDays = api.impress.getByPath(application.config, 'log.keepDays');
    if (!keepDays) {
      keepDays = api.impress.getByPath(impress.config, 'log.keepDays');
    }
      ...
      if (application === impress || applicationLog) {
        api.mkdirp(application.log.dir, function(err) {
          if (err) {
            ...
            var now = new Date(),
                nextDate = new Date();
            api.impress.each(application.log.fileTypes, function(fileType, cb) {
            ...
            if (keepDays && api.cluster.isMaster) {
