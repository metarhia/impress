...
class Application extends events.EventEmitter {
  ...
  async init(kind) {
    ...
    this.startWatch();
    this.createSandbox();
    this.sandbox.application.emit('init');
    ...
    this.sandbox.application.emit('init-loaded');
    await Promise.allSettled(this.starts.map((fn) => this.execute(fn)));
    this.sandbox.application.emit('init-started');
    await this.api.load();
    ...
    this.sandbox.application.emit('init-done');
  }
