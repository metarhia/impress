    ...
    api.mkdirp(targetDir, function(err) {
      ...
      fd.on('end', function() {
        ...
        if (data.compressionFlag === 'Z') {
          ...
          arc.pipe(out);
          arc.on('end', function() { client.saveUploadedFile(data, onFile); });
          ...
          inp.on('end', function() { client.saveUploadedFile(data, onFile); });
...
Client.prototype.saveUploadedFile = function(data, onFile) {
