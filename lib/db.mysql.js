...
api.mysql = api.impress.require('mysql', true);
api.mysqlUtilities = api.impress.require('mysql-utilities', true);

if (api.mysql) {

  db.mysql = {};
  db.mysql.schema = {};
  db.drivers.mysql = api.mysql;

  if (api.mysqlUtilities) {
    db.mysql.upgrade = api.mysqlUtilities.upgrade;
    db.mysql.introspection = api.mysqlUtilities.introspection;
  }
      ...
      for (tableName in databaseSchema) {
        if (tableName !== 'caption' && tableName !== 'version') {
          ...
          for (fieldName in table.fields) {
            ...
            if (field.primary || (fieldIndex === 1 && api.impress.inArray(['id', 'uid'], field.type))) primaryKey.push(fieldName);
            if (field.index) {
            ...
            fieldsDef += '  ' + api.impress.rpad(fieldName, ' ', fieldNameMaxLength + 1) + ' ' + fieldDef + ',' + crlf;
