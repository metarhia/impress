			...
			if (err) console.error(err);
				...
				for (var i in impress.log.fileTypes) {
					...
					impress.log.files[fileType] = {
						fd: impress.fs.createWriteStream(fileName, {flags: 'a'}),
						buf: "",
						lock: false,
						timer: setInterval(makeTimer(fileType), impress.config.log.writeInterval || defaultWriteInterval)
					};
				}
