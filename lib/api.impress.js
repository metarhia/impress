...
api.impress.subst = function(tpl, data, dataPath, escapeHtml) {
  return tpl.replace(SUBST_REGEXP, function(s, key) {
    ...
    if (value === undefined) {
      if (key === '.value') {
        value = api.impress.getByPath(data, dataPath);
      } else {
        value = '[undefined]';
      }
    }
    if (value === null) {
      ...
      if (value.constructor.name === 'Date') {
        value = api.impress.nowDateTime(value);
...
//
api.impress.newEscapedRegExp = function(s) {
  return new RegExp(api.impress.escapeRegExp(s), 'g');
};
...
api.impress.bytesToSize = function(bytes) {
  ...
  return (
    ...
    api.impress.bytesToSize.sizes[i]
  );
};

api.impress.bytesToSize.sizes = [
...
api.impress.clone = function(obj, ext) {
  ...
  for (var i in obj) {
    if (obj[i] && typeof(obj[i]) === 'object') {
      copy[i] = api.impress.clone(obj[i]);
    } else {
      copy[i] = obj[i];
    }
  }
  if (ext !== null && typeof(ext) === 'object') {
    for (var j in ext) {
      if (ext[j] && typeof(ext[j]) === 'object') {
        copy[j] = api.impress.clone(ext[j]);
      } else {
        copy[j] = ext[j];
      }
    }
...
//
api.impress.require = function(moduleName, soft) {
  ...
  try {
    ...
    if (api.cluster.isMaster) {
      ...
      if (impress.log && typeof(impress.log.warning) === 'function') {
...
STACK_REGEXP.forEach(function(item, i) {
  if (typeof(item[0]) === 'string') {
    item[0] = api.impress.newEscapedRegExp(item[0]);
  }
});
