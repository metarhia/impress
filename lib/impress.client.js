...
const extCases = {
  ...
  sse: (client) => {
    if (!client.application.sse) return client.error(510);
    client.sseConnect();
  },

  ws: (client) => {
    if (!client.application.websocket) return client.error(510);
    client.application.websocket.finalize(client);
  },

  json: (client) => {
    if (!client.context.data) return client.error(400);
  ...
  csv: (client) => {
    ...
    api.csv.stringify(client.context.data, (err, data) => {
      if (err) return client.error(500, err);
      client.end(data);
    });
  }
...
Client.prototype.destroySession = function() {
  ...
  if (application && client.session) {
    ...
    application.security.deleteSession(client.sid, api.common.emptiness);
    client.sid = null;
    client.user = null;
    client.session = null;
  }
...
Client.prototype.restoreSession = function(
  ...
  if (!application || !sid) return callback();
...
Client.prototype.saveSession = function(callback) {
  ...
  if (!application) return callback();
  const cfg = application.config.sessions
  if (cfg && cfg.persist && client.session) {
    if (client.sessionCreated) {
      return application.security.createSession(client.session, done);
    }
    if (client.sessionModified) {
      return application.security.updateSession(client.session, done);
    }
  }
  callback();
...
Client.prototype.processing = function() {
  ...
  api.metasync.series(handlers, (handler, cb) => {
    ...
    if (!err && !client.res.headersSent) {
      if (!client.access.allowed) return client.error(403);
...
Client.prototype.runScript = function(
  ...
  application.createScript(fileName, (err, fn) => {
    ...
    if (handler === 'lazy') {
      const cb = callback;
      callback = api.common.emptiness;
      api.timers.setImmediate(() => client.executeFunction(fn, callback));
      cb();
    } else {
...
Client.prototype.executeFunction = function(
  ...
  if (typeof(fn) !== 'function') return callback();
  try {
...
Client.prototype.staticFile = function(
  ...
  api.fs.readFile(filePath, (err, data) => {
    if (err) return client.error(404);
...
Client.prototype.buffer = function(
  ...
  console.dir({buffer:time, stats:cache.stats.mtime});
...
Client.prototype.compress = function(
  ...
  console.dir({compress:time, stats:stats.mtime});
  if (notMod) return client.error(304);
...
Client.prototype.signOut = function(
  ...
  if (!client.session) return callback(false);
