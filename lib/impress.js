		...
		for (var i in apps) {
			...
			(function() {
				if (stats.isDirectory() && (!impress.workerApplicationName || impress.workerApplicationName === appName)) {
					...
					application.Client = impress.createApplicationClientClass({});
	...
	if (api.cluster.isMaster) {
		...
		process.on('message', function(message, socket) {
			if (message.name === 'impress:socket') {
				...
				if (application) {
					/**/ if (message.user)    application.sse.sendToUser(null, message.user, message.event, message.data, true);
					else if (message.channel) application.sse.sendToChannel(null, message.channel, message.event, message.data, true);
					else if (message.global)  application.sse.sendGlobal(null, message.event, message.data, true);
				}
			}
		});
	}
