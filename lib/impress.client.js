...
var Client = function(application, req, res) {
  ...
  if (res.setHeader) {
    res.setHeader('Content-Type', impress.mimeTypes[this.typeExt]);
...
impress.createApplicationClientClass = function(application, mixin) {
  ...
  var property;
  for (property in mixin) ApplicationClientClass.prototype[property] = mixin[property];
...
Client.prototype.end = function(output, socket) {
  ...
  client.saveSession(function() {
    ...
    if (!isBuffer && !client.res.headersSent && length > impress.compressAbove) {
...
Client.prototype.detectRealPath = function(callback) {
  ...
  if ((folderCache && folderCache !== impress.FILE_EXISTS) || client.realPath === '/') callback();
...
Client.prototype.static = function() {
  if (api.path.basename(this.path) === 'access.js') this.error(403);
    ...
    if (buffer) {
      if (buffer !== impress.FILE_NOT_FOUND) {
