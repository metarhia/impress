...
class StaticCache {
  ...
  async loadPlace(name) {
    ...
    const files = new Map();
    this.places.set(name, { files, path: dirPath, entriesCache: null });
  ...
  static getPlaceEntries(cache, name) {
    ...
    if (!place) return [];
    if (place.entriesCache) return place.entriesCache;
    const entries = Array.from(place.files.values());
    place.entriesCache = entries;
    return entries;
  }
    ...
    this.watcher.on('change', (filePath) => {
      ...
      node.fs.stat(filePath, async (err, stat) => {
        ...
        if (stat.isDirectory()) {
          ...
          const before = new Set(this.places.get(name).files.keys());
          await this._loadDir(name, filePath);
          const place = this.places.get(name);
          place.entriesCache = null;
          for (const [key, entry] of place.files) {
            if (!before.has(key)) {
        ...
        const entry = await this._loadFile(name, filePath);
        if (entry) {
          this.places.get(name).entriesCache = null;
          broadcast({ name: 'cache-update', place: name, entry });
        }
      });
    });
