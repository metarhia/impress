...
api.registry.npmNameToApiName = (npmName) => {
  let m = api.registry.indexByNpm[npmName];
...
api.registry.apiNameToNpmName = (apiName) => {
  let m = api.registry.modules[apiName];
...
//
api.registry.find = (name) => {
  let m = api.registry.modules[name];
  if (m) return m;
...
//
api.registry.require = (moduleName, soft) => {
  let mr = api.registry.find(moduleName);
  let npmName;
