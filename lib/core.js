...
const loadApplications = callback => {
  ...
  const server = impress.config.sections.servers[impress.serverName];
...
impress.start = () => {
  ...
  impress.createSandbox(impress, () => {
    impress.loadConfig(() => {
      ...
      const cfg = impress.config.sections.scale;
...
impress.load = () => {
  ...
  loadApplications(() => {
    ...
    Object.keys(impress.config.sections.servers).forEach(key => {
      const server = impress.config.sections.servers[key];
