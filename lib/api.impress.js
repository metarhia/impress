...
api.impress.clearCacheStartingWith = function(cache, startsWith, callback) {
  for (var key in cache) if (api.impress.startsWith(key, startsWith)) {
    delete cache[key];
...
api.impress.spinalToCamel = function(name) {
  var namePar = name.replace(/_/g, '-').split('-');
...
api.impress.generateKey = function(length, possible) {
  var key = '';
  for (var i = 0; i < length; i++) key += possible.charAt(api.impress.random(0, possible.length - 1));
  return key;
};
...
api.impress.ip2int = function(ip) {
  ...
  return ip.split('.').reduce(function(res, item) {
    return (res << 8)+ +item; // jshint ignore:line
  }, 0);
};
...
api.impress.escapeRegExp = function(s) {
  return s.replace(api.impress.escapeRegExp.regex, '\\$&');
};
...
api.impress.escapeRegExp.regex = new RegExp('[' + api.impress.escapeRegExp.specials.join('\\') + ']', 'g');

// Add single slash to the right with no duplicates
//
api.impress.trailingSlash = function(s) {
  return s + (s.slice(-1) === '/' ? '' : '/');
};
...
// Print message and call function if specified
//
api.impress.log = function(msg, substring) {
  console.log(msg);
