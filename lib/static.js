...
const write = (res, data, code, ext = 'html', options = {}) => {
...
class Static extends Place {
  ...
  async serve(url, req, res) {
    ...
    if (file.data && file.stat) {
      if (file.code === -1) return void write(res, file.data, 200, 'html');
      return void write(res, file.data, file.code, fileExt);
    }
      ...
      if (stat && stat.isFile()) {
        ...
        if (req.headers.range) {
          ...
          if (start >= end || start >= size || end >= size) {
            ...
            return void write(res, file.data, 416, fileExt);
          }
          options.start = start;
          options.end = end;
          code = 206;
        ...
        return void write(res, readable, code, fileExt, options);
      }
    }
    if (file.code === -1) return void write(res, file.data, 200, 'html');
    return void write(res, file.data, 404);
  }
}
