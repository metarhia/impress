  ...
  impress.scale.init = function() {
    if (api.cluster.isMaster && impress.config.scale) {
      ...
      if (cs.instance === 'controller') {
        ...
        impress.scale.pub.bind('tcp://' + cs.host + ':' + cs.subPort, function(err) {
        ...
        impress.scale.rep.bind('tcp://' + cs.host + ':' + cs.reqPort + '', function(err) {
          if (err) console.log(err);
        ...
        impress.scale.req.on('message', function(reply) {
          ...
          if (msg.name === 'impress:scale:id') {
            ...
            for (var workerId in api.cluster.workers) api.cluster.workers[workerId].send(msg);
          }
        });
        impress.scale.req.connect('tcp://' + cs.host + ':' + cs.reqPort);
        ...
        // impress.scale.controller.lastCheck = null;       // Date.now()
