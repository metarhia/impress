...
//   httpOnly:boolean - add HttpOnly cookie modifier if true (optional)
//
...
//   host:string - forward request to nost name or IP address
//   port:number - request port number
...
//   session:object - object/hash with session fields
//
...
//   inheritance:boolean - true if called from Client.prototype.inherited
    ...
    if (buffer) {
      ...
      if (err) {
        ...
        if (sinceTime && api.impress.isTimeEqual(sinceTime, stats.mtime)) client.error(304);
          ...
          var appPath = api.impress.stripTrailingSlash(application.dir + '/app' + client.path);
