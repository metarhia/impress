...
function walk(parent, data, definition, name, path, callback) {
  var tpl = definition[name],
      i, j, key, subKey, item, itemType, template, def, itemPath, subItem, subItemPath, other;
...
api.definition.preprocess = function(data, definition, definitionName, path) {
  ...
  api.definition.walk(data, definition, name, api.definition.preprocessItem);
  return result;
};
...
//   preprocess - flag for data preprocessing
//
api.definition.validate = function(data, definition, name, preprocess) {
  ...
  api.definition.walk(data, definition, name, function(parent, data, definition, name, path, def) {
    if (preprocess) api.definition.preprocessValue(parent, data, definition, name, path, def);
    ...
    if (def.type !== 'undefined') {
      ...
      if (!itemValid) {
        //console.dir([path,def]);
        result.errors.push({ path: path, error: 'Unexpected value "'+data+'" for definition: '+def.str });
      }
