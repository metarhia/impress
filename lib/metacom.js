...
class Server {
  constructor(config, { application, Client }) {
    ...
    const isBalancer = balancer && threadId === 1;
    const port = isBalancer ? balancer : ports[threadId - 1];
    const transport = protocol === 'http' || isBalancer ? http : https;
  ...
  listener(req, res) {
    ...
    if (!req.connection.encrypted) {
      ...
      const port = common.sample(this.ports);
