...
function mixinApplication(application, callback) {
	...
	//
	application.preloadDirectory = function(relPath, depth) {
		if (application.config.files && application.config.files.staticRx) {
			var staticRx = application.config.files.staticRx;
			if (staticRx && staticRx.test(relPath)) return;
		}
		if (typeof(dapth) === 'undefined') depth = 0;
				...
				for (var i = 0; i < files.length; i++) {
					(function() {
						var fileName = files[i],
							fileExt = impress.fileExt(fileName),
							filePath = trailingSlash(absPath)+fileName;
						api.fs.stat(filePath, function(err, stats) {
							if (!err) {
								if (stats.isDirectory()) {
									if (depth === 0 || depth>1) application.preloadDirectory(trailingSlash(relPath)+fileName, depth-1);
								} else if (fileExt === 'js') application.createScript(null, filePath);
							}
						});
					} ());
				}
