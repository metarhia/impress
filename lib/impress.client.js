...
Client.prototype.defaultContentType = function() {
  ...
  var allowOrigin = api.impress.getByPath(
    application.config,
...
Client.prototype.detectRealPath = function(callback) {
  ...
  if ((folderCache && folderCache !== impress.DIR_NOT_EXISTS) || client.realPath === '/') callback();
    ...
    application.cache.watch(api.impress.stripTrailingSlash(relPath));
  });
};
...
Client.prototype.serveStatic = function(relPath, onNotServed) {
  ...
  var gz = '.gz',
      ...
      isCompressed  = api.impress.inArray(impress.COMPRESSED_EXT, client.ext);
  ...
  function serveFile(err, stats) {
    if (err) {
      ...
      application.cache.watch(api.path.dirname(relPath));
      var isIndex = api.impress.endsWith(relPath, '/index.html');
...
Client.prototype.buffer = function(cache) {
  ...
  if (sinceTime && api.impress.isTimeEqual(sinceTime, cache.stats.mtime)) {
    client.error(304);
