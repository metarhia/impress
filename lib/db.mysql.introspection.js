(function(db) {
	...
	db.mysql.introspection = function(connection) {

		connection.primary = function(table, callback) {
			this.queryRow('SHOW KEYS FROM '+db.escape(table)+' WHERE Key_name = "PRIMARY"', [], function(err, res) {
				if (err) res = false;
				callback(err, res);
			});
		}
		...
		connection.fields = function(table, callback) {
			this.queryHash('SHOW FULL COLUMNS FROM '+db.escape(table), [], function(err, res) {
				if (err) res = false;
				callback(err, res);
			});
		}
		...
		connection.indexes = function(table, callback) {
			this.query('SHOW INDEX FROM '+db.escape(table), [], function(err, res) {
				var result = {};
