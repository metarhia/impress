...
api.jstp.CHUNKS_FIRST = api.common.buffer('[');
api.jstp.CHUNKS_LAST = api.common.buffer(']');
api.jstp.HANDSHAKE_TIMEOUT = 3000;
api.jstp.PARSE_TIMEOUT = 30;
...
api.jstp.Chunks.prototype.add = function(chunk) {
  ...
  if (delimiter === api.jstp.PACKET_DELIMITER) {
    ...
    this.items.push(api.common.buffer(chunk));
    return null;
  }
};
...
// Inverse function: api.jstp.parse
//
...
// Inverse function: api.jstp.interprete
//
...
//   str - object serialized to string
//   return - deserialized JavaScript object
// Inverse function: api.jstp.dump
//
...
// Inverse function: api.jstp.deserialize
//
api.jstp.serialize = function(a, i, arr) {
  ...
  if (a instanceof Array) {
    ...
    return a;
    // a may be number, boolean, string, etc.
...
// Inverse function: api.jstp.serialize
//
...
api.jstp.connect = function(name, host, port, secure) {
  var provider = secure ? api.tls : api.net;
  var connection = null;

  var socket = provider.connect({
    port: port,
    host: host,
  }, function() {
    //socket.write();
    ...
    handshake: function () {
      ...
      if (packet.ok) {
        ...
        if (application) {
          ...
          connection.on('packet', function(packet, connection) {
            ...
            application.log.api(msg);
          });

          // TODO: check credentials
          connection.emit(
            'handshakeRequest', application, username, password, connection
          );
