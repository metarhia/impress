...
api.db.openApplicationDatabases = (application, callback) => {
  ...
  let databases = application.config.databases;
  let names = Object.keys(databases);
...
api.db.schema.validate = (databaseSchema, consoleOutput) => {
  let result = api.definition.validate(
