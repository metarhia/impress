...
const mixin = application => {
  ...
  application.cache = new impress.Cache(application);
  application.cache.init();

  if (!application.isImpress) {
    api.firewall.addApplication(application);
    application.state = new impress.State();
    application.security = new impress.Security(application);
    application.cloud = new api.cloud.CloudEmitter(application);
    // SSE connections
    application.sse = new impress.ServerSentEvents(application);
    // JSTP connections
  ...
  application.startWorker = (client, workerFile) => {
  ...
  application.stopWorker = (client, workerFile) => {
    const fileName = client.pathDir + workerFile + '.js';
