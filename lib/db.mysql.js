	...
	db.mysql.upgrade = function(connection) {
		...
		connection.query = connection.query.override(function(sql, values, callback) {
			...
			var query = this.inherited(sql, values, function(err, res, fields) {
				...
				if (endTime-startTime >= connection.slowTime) impress.log.slow((endTime-startTime)+'ms\t'+query.sql);
		...
		connection.count = function(table, where, callback) {
			var where = this.where(where),
				sql = 'SELECT count(*) FROM '+db.escape(table);
		...
		connection.select = function(table, fields, where, callback) {
			var where = this.where(where),
				sql = 'SELECT '+fields+' FROM '+db.escape(table);
			if (where)  sql = sql+ ' WHERE '+where;
		...
		connection.insert = function(table, row, callback) {
			...
			dbc.fields(table, function(err, fields) {
				if (!err) {
					...
					var query = dbc.query('INSERT INTO '+db.escape(table)+' ('+columns+') VALUES ('+values+')', [], function(err, res) {
		...
		connection.update = function(table, row, callback) {
			...
			dbc.fields(table, function(err, fields) {
				if (!err) {
					...
					if (where) {
						data = data.join(', ');
						var query = dbc.query('UPDATE '+db.escape(table)+' SET '+data+' WHERE '+where, [], function(err, res) {
			...
			dbc.fields(table, function(err, fields) {
				if (!err) {
					...
					if (rowKeys.indexOf(uniqueKey)!=-1) {
						dbc.queryValue('SELECT count(*) FROM '+db.escape(table)+' WHERE '+uniqueKey+'='+dbc.escape(row[uniqueKey]), [], function(err, count) {
		...
		connection.delete = function(table, where, callback) {
			var where = this.where(where);
			if (where) var query = this.query('DELETE FROM '+db.escape(table)+' WHERE '+where, [], function(err, res) {
