		...
		if (application.config.databases) {
			...
			for (var databaseName in databases) {
				...
				if (driver) driver.open(database, function() {
					if (database.alias && application.sandbox) application.sandbox[database.alias] = database.connection;
					if (database.global && application.sandbox) application.sandbox[databaseName] = database.connection;
					cb();
				}); else {
