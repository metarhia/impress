(function(db) {
	...
	db.mysql.upgrade = function(connection) {
		...
		connection.query = connection.query.override(function(sql, values, cb) {
			var query = this.inherited(sql, values, cb);
		...
		connection.count = function(table, where, callback) {
			...
			return this.queryValue(sql, [table], function(err, res) {
				callback(err, res);
			});
		}
		...
		connection.select = function(table, fields, where, callback) {
			...
			if (where)  sql = sql+ ' WHERE '+where;
			var query = this.query(sql, [table], function(err, res) {
		...
		connection.insert = function(table, row, callback) {
			...
			dbc.fields(table, function(err, fields) {
				if (!err) {
					...
					var query = dbc.query('INSERT INTO ?? ('+columns+') VALUES ('+values+')', [table], function(err, res) {
		...
		connection.update = function(table, row, callback) {
			...
			dbc.fields(table, function(err, fields) {
				if (!err) {
					...
					if (where) {
						data = data.join(', ');
						var query = dbc.query('UPDATE ?? SET '+data+' WHERE '+where, [table], function(err, res) {
		...
		connection.delete = function(table, where, callback) {
			var where = this.where(where);
			if (where) var query = this.query('DELETE FROM ?? WHERE '+where, [table], function(err, res) {
