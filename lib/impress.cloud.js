...
impress.cloud.createServer = function() {
  ...
  var server = impress.config.servers.cloud;
  if (server) {
    //impress.setListenerError(server.instance, server.host + ':' + server.ports[0]);
    ...
    server.instance.on('handshake', function(session, connection) {
      console.log('Cloud RPC server.instance.on(handshake), TODO');
...
impress.cloud.createClient = function() {
  ...
  //
  // A temporary workaround around incorrect Cloud Controller address,
  // with the above line workers die after implementing `error` event for
  // JSTP connections since they fail to connect and the event is being
  // caugth by the domain
  // TODO: @tshemsedinov, please fix it appropriately
  //
  var client = api.jstp.connect('cloud', impress.config.servers.master.host,
                                         impress.config.servers.master.port);
