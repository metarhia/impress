...
api.db.openApplicationDatabases = function(application, callback) {
  ...
  if (databases && Object.keys(databases).length > 0) {
    api.metasync.each(Object.keys(databases), function(databaseName, cb) {
...
api.db.openDatabase = function(application, database, callback) {
  ...
  database.schema = (database.opt) ? database.opt.schema : database.url.substr(0, database.url.indexOf(':'));  if (database.schema === 'postgres') database.schema = 'pgsql';
