...
// Upload file
//   onFile(fileName) - callback on processing each file
//
            ...
            fd.on('end', function() {
              ...
              if (compressionFlag === 'Z') {
                var zipFile = storageFile + '.zip',
                    ...
                    zipOut = api.fs.createWriteStream(zipFile),
                    zipInp = api.fs.createReadStream(storageFile);
                archive.pipe(zipOut);
                archive.on('end', function() {
                  api.fs.unlink(storageFile, function() {
                    api.fs.rename(zipFile, storageFile, function() {
                      if (onFile) onFile(storageFile);
                    });
                  });
                });
                ...
                var gzipFile = storageFile + '.gz',
                    gzip = api.zlib.createGzip(),
                    gzipInp = api.fs.createReadStream(storageFile),
                    gzipOut = api.fs.createWriteStream(gzipFile);
                gzipInp.pipe(gzip).pipe(gzipOut);
                gzipInp.on('end', function() {
                  api.fs.unlink(storageFile, function() {
                    api.fs.rename(gzipFile, storageFile, function() {
                      if (onFile) onFile(storageFile);
                    });
                  });
                });
