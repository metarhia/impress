...
var Client = function(application, req, res) {
  ...
  this.access = clone(impress.DEFAULT_ACCESS);
  this.calculateAccess();
  this.query = api.querystring.parse(url.query);
  this.parameters = {};
  extend(this.parameters, this.query);
...
impress.createApplicationClientClass = function(application, mixin) {
  ...
  application.Client = ApplicationClientClass;
...
// Client class mixin
//
Client.prototype.mixin = function(mixin) {
  for (var property in mixin) this.application.Client.prototype[property] = mixin[property];
};
...
Client.prototype.fork = function(workerFile) {
  if (this.application) {
    ...
    impress.forkLongWorker(this.application.name, fileName, clientData);
  }
};
...
Client.prototype.startSession = function() {
  if (this.application && !this.session) {
    ...
    this.setCookie(this.application.config.sessions.cookie, this.sid, this.application.config.sessions.domain);
    if (impress.config.scale.cookie) this.setCookie(impress.config.scale.cookie, impress.nodeId);
...
Client.prototype.destroySession = function() {
  if (this.application && this.session) {
    this.deleteCookie(this.application.config.sessions.cookie, this.application.config.sessions.domain);
    this.deleteCookie(impress.config.scale.cookie, this.application.config.sessions.domain);
...
Client.prototype.deleteCookie = function(name, host) {
  host = host || this.req.headers.host;
  this.cookies.push(name + '=deleted; Expires=Thu, 01 Jan 1970 00:00:01 GMT; Path=/; Domain=.' + host);
};
...
Client.prototype.dispatcher = function() {
  ...
  client.restoreSession(function() {
    application.before.emit(client, function() {
      client.processing();
    });
  });
};
...
Client.prototype.processing = function() {
  ...
  client.detectRealPath(function() {
    ...
    if (cache && (client.startTime < cache.expireTime)) {
      ...
      client.handlers = [ 'access', 'request', client.method, 'end', 'lazy' ];
      client.context = {};
...
Client.prototype.end = function(output, socket) {
  ...
  client.saveSession(function() {
    ...
    impress.stat.responseCount++;
    if (client.dynamicHandler) application.after.emit(client);
...
Client.prototype.error = function(code, es) {
  ...
  if (es instanceof Error) client.err = es;
  else var socket = es;
  client.res.statusCode = code;
  client.fileHandler('error', function() {
...
Client.prototype.fileHandler = function(file, callback) {
  ...
  if (fileCache) {
    if (fileCache !== impress.FILE_NOT_FOUND) client.execute(file, fileCache, callback);
    else callback();
  } else api.fs.exists(filePath, function(exists) {
    var fileOriginal;
    if (exists) {
      client.execute(file, filePath, callback);
...
//
Client.prototype.execute = function(handler, filePath, callback) {
  var client = this,
      application = client.application;

  if (client.access.allowed && client.application) {
    if (handler === 'lazy') setImmediate(function() {
      application.runScript(filePath, client, function() {});
      callback();
    }); else application.runScript(filePath, client, callback);
  } else callback();
};
  ...
  if (cache) {
    ...
    api.fs.readFile(filePath, 'utf8', function(err, tpl) {
      if (err) callback(impress.TPL_NOT_FOUND + filePath);
        ...
        if (!tpl) tpl = impress.FILE_IS_EMPTY; else {
          tpl = tpl.replace(/^[\uBBBF\uFEFF]/, '');
  ...
  if (tpl !== impress.FILE_IS_EMPTY) {
    ...
    while (tpl.length > 0) {
      ...
      if (pos >= 0) {
        ...
        tpl = tpl.substring(pos + 2);
        dataInclude = impress.getByPath(data, (cursor ? cursor + '.' : '') + tplInclude);
