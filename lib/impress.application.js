...
impress.application.mixin = function(application) {
  ...
  application.isMaster = api.process.isMaster;
  application.isWorker = api.process.isWorker;
      ...
      if (!application.config[sectionName]) {
        application.createScript(configFile, function(err, exports) {
          ...
          if (configDefinition[sectionName]) {
            ...
            if (api.process.isMaster) api.definition.printErrors(
  ...
  application.preprocessConfig = function() {
    ...
    for (serverName in servers) {
      ...
      for (i = 0, ilen = server.ports.length; i < ilen; i++) {
        ...
        srv.port = port;
        config.servers[serverName + port] = srv;
      }
    }
  ...
  application.processPlaceFile = function(placeName, placeDir, file, load, pass) {
    ...
    if (placeName === 'setup') {
      ...
      if (api.process.isWorker) return pass();
  ...
  application.startTask = function(taskName) {
    ...
    if (task && !task.active) {
      if (
        (api.process.isMaster && task.place === 'master') ||
        (api.process.isWorker && task.place === 'worker')
      ) {
        task.active = true;
