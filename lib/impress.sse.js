...
impress.sse.mixinApplication = function (application) {
	...
	//
	application.Client.prototype.sse.connect = function() {
		var client = this;
		...
		if (client.sse.channel && !client.res.headersSent) {
			...
			applications.sse.connections[connectionId] = client;

			if (!application.sse.channels[client.sse.channel]) applications.sse.channels[client.sse.channel] = [];
	...
	//
	application.Client.prototype.sse.sendToUser = function(client, userId, eventName, data, isRetranslation) {
	...
	//
	application.Client.prototype.sse.sendToChannel = function(client, channel, eventName, data, isRetranslation) {
		var buf = impress.sse.packet(eventName, data),
			isRetranslation = isRetranslation || false;
	...
	//
	application.Client.prototype.sse.sendGlobal = function(client, eventName, data, isRetranslation) {
		var buf = impress.sse.packet(eventName, data);
		isRetranslation = isRetranslation || false;
