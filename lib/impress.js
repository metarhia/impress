(function(impress) {
	...
	//   4. persistent and liquid sessions
	//   5. cluster identification cookies for loadbalancing
	//   6. advanced folder monitoring (replace pool)
	//   7. req.post and req.query
	//   8. return 404 for static foles in processing directories e.g. http://mezha.net/static/v.1.0pre/_demo/root.png

	// todo:
	//   1. offline applications .manifest
	//   2. file uploader
	//   3. IP geo lookup
	//   4. jb drivers for webSQL, indexedDB
	//   5. error pages
	//   6. /api/auth
	//   7. web GUI controls
	//   8. handlers: .xml, .form
	//   9. cms with mongoDB storage
	//  10. mongoDB backup/restore
	//  11. static files memory caching
	//  12. simple email sender
	//  13. graceful shutdown
	//  14. allow access according to access.js files
	//  15. hide internal methods
	//  16. 
	//  17. 

	// errors:
	//   1. config reload
	//   2. run second copy of impress
	//   3. 
	//   4. 
	...
	impress.sendCookie = function(req, res) {
		if (res.impress.cookies.length) res.setHeader("Set-Cookie", res.impress.cookies);
	}
	...
	//
	impress.register = function(email, callback) {
		impress.getUser(email, function(err, node) {
			if (!node) {
				db.impress.users.insert({
					login: email,
					password: "111"
				}, function(err, nodes) {
					// if (callback) callback(null, rootNode);
				});
			}
		});
	}
	...
	impress.route = function(req, res, host, port, url) {
		impress.http.request({
			...
			res.end("HTTP request error: "+err.message);
		}).end();
	}
	...
	impress.saveSession = function(req, res, callback) {
		...
		if (req.impress.sessionModified) db.impress.sessions.insert(session, callback);
		else if (req.impress.sessionCreated) db.impress.sessions.update({ sid: req.impress.session }, session, callback);
		else callback();
	}
	...
	impress.process = function(req, res) {	
		...
		var contentType = impress.config.mimeTypes[req.impress.ext];
		...
		async.eachSeries(req.impress.handlers, function(handler, callback) {
			...
			if (req.impress.ext == 'html') {
				...
				var data = req.context.data || {};
				impress.template(req, res, data, 'html', '', function(tpl) {
					impress.end(req, res, tpl);
				});
			} else {
				var output = '';
				if (req.impress.ext == 'json') output = JSON.stringify(req.context.data)
				else if (req.impress.ext == 'xml') output = objectToXml(req.context.data.toXml);
				if (!output) impress.error(req, res, 404);
				impress.end(req, res, output);
			}
		});
	}
	...
	impress.static = function(req, res) {
		var filePath = req.impress.hostDir+req.impress.path,
			...
			httpCode = impress.config.customHttpCodes[req.impress.ext] || 200;
		impress.fs.stat(filePath, function(err, stats) {
			...
			if (ifSince && stats) {
				...
				if (since === now) {
					res.writeHead(304); // 'HTTP/1.1 304 Not Modified'
					res.end();
					return;
				}
			}
	...
	impress.baseHeader = function(ext, stats) {
		var header = {
			...
			'Content-Type':      impress.config.mimeTypes[ext]
