...
impress.normalizeStack = (stack) => {
  impress.STACK_REGEXP.forEach((rx) => {
...
impress.fatalError = (
  ...
  if (msg.includes('application.createScript')) {
    ...
    console.dir({
      impresslogserver: impress.log.server,
      type: typeof(impress.log.server)
    });
    //impress.log.server('Crashed');
    impress.log.close(() => impress.shutdown(1));
  } else {
    impress.shutdown(1);
  }
};
...
impress.loadApplications = (callback) => {
  ...
  if (impress.workerApplications) {
    return impress.createApplications(callback);
  }
  ...
  impress.createSandbox(() => {
    impress.loadConfig(() => {
      ...
      if (!process.isMaster || !cfg || !cfg.check) {
        return impress.server.load();
      }
      console.log('Startup check: '.green + cfg.check);
