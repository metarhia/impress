...
api.common.generateKey = (length, possible) => {
  ...
  let bitsCount = Math.ceil(Math.log2(Math.pow(base, length + 1) - 1));
  let bytes = api.crypto.randomBytes(Math.ceil(bitsCount / 8));
  let key = '';
  for (let i = 0; i < length; i++) {
    let index = api.common.longDivModBE(bytes, base);
    key += possible[index];
  }
  return key;
};
...
api.common.longDivModBE = (buffer, divisor) => {
  ...
  let bytesCount = Buffer.byteLength(buffer);
  let remainder = 0;
...
api.common.generateGUID = () => {
  let bytes = api.crypto.randomBytes(128);
...
//
api.common.ip2int = (ip) => {
  if (!ip) ip = '127.0.0.1';
  return ip.split('.').reduce((res, item) => {
    let num = +item;
    return (res << 8) + num;
  }, 0);
};
...
api.common.bytesToSize = (bytes) => {
  if (bytes === 0) return '0';
  let i = parseInt(Math.floor(Math.log(bytes) / Math.log(1000)), 10);
  return (
...
api.common.sequence = (seq, max) => {
  let from = seq[0],
      ...
      res = seq;
  if (Array.isArray(to)) {
    let count = to[0] < 0 ? max + to[0] : to[0];
...
//
api.common.isScalar = (variable) => (
  SCALAR_TYPES.indexOf(typeof(variable)) !== -1
);

// Define inArray, if not implemented
//
api.common.inArray = (array, value) => (
  array ? array.indexOf(value) !== -1 : false
);

// Merge arrays into first one
//
api.common.merge = (...args) => {
  let i, ilen, j, jlen, arr,
      array = args[0];
...
//
api.common.buffer = (s, encoding = 'utf8') => (
...
api.common.localIPs = () => {
  ...
  if (ips) {
    ...
    ips = [];
    let protocol, ifItem, ifHash = api.os.networkInterfaces();
    for (let ifName in ifHash) {
      ifItem = ifHash[ifName];
      for (let i = 0, len = ifItem.length; i < len; i++) {
        protocol = ifItem[i];
...
api.common.generateSID = (config) => {
  let key = api.common.generateKey(
...
api.common.crcSID = (config, key) => {
  let md5 = api.crypto.createHash('md5');
...
api.common.validateSID = (config, sid) => {
  if (!sid) return false;
  let crc = sid.substr(sid.length - 4),
...
api.common.logApiMethod = (fnPath) => {
  let originalMethod = api.common.getByPath(api, fnPath);
  api.common.setByPath(api, fnPath, (...args) => {
    ...
    let logArgs = api.json.stringify(args);
