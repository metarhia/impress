...
Client.prototype.staticCache = function(relPath) {
  ...
  var buffer = application.cache.static[relPath],
      cached = buffer && typeof(buffer) !== 'number';
  if (cached) client.buffer(buffer);
  return cached;
};
...
//   buffer - buffer with additional fields
//     buffer.stats - stats structure
//     buffer.compressed - gzip compression flag
//     buffer.data - data to send
//
Client.prototype.buffer = function(buffer) {
  var client = this;
