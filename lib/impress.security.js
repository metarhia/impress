...
impress.security.mixinApplication = function(application) {
  ...
  // Mixin security dabase provider
  //
  if (application.databases && application.databases.security) {
    var provider = impress.security.getDatabaseProvider(application.databases.security.schema);
    if (provider) provider.mixinApplication(application);
  }
