          ...
          if (exists) {
            watcher = api.fs.watch(path, function(event, fileName) {
              ...
              if (application.cache.updates.indexOf(filePath) === -1) {
                (function() {
                  var updatesPath = filePath;
                  api.fs.exists(updatesPath, function(exists) {
                    if (exists && application.cache.updates.indexOf(updatesPath) === -1) {
                      application.cache.updates.push(updatesPath);
                    }
                  });
                } ());
              }
              application.cache.timer = setTimeout(function() {
                var updates = application.cache.updates;
                for (i = 0; i < updates.length; i++) {
                  (function() {
                    var filePath = updates[i];
                    api.fs.stat(filePath, function(err, stats) {
                      if (err) return;
                      if (stats.isFile()) application.updateFileCache(filePath, stats);
                      else application.cliearDirectoryCache(filePath);
                      // Implement: application.updateDirectoryCache(filePath)
                    });
                  } ());
                }
