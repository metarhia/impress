...
const handlers = {
  'cache-init': ({ place, entries }) => {
    const target = application[place];
    if (target) target.initCache(entries);
  },

  'cache-update': ({ place, entry }) => {
    const target = application[place];
    if (target) target.updateEntry(entry);
  },

  'cache-delete': ({ place, key }) => {
    const target = application[place];
    if (target) target.deleteEntry(key);
  },
