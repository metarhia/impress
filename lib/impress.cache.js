...
impress.cache.mixin = (application) => {
  ...
  cache.init = () => {
    ...
    cache.files = new Map(); // file override/inherited cache indexed by file name
    ...
    cache.static = new Map(); // static files cache
  ...
  application.cliearDirectoryCache = (
    ...
    api.common.clearCacheStartingWith(cache.files, relPath, (used) => {
      ...
      } else if (ext === 'template' && cache.templates.has(used)) {
