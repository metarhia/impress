...
Client.prototype.processing = function() {
  ...
  client.detectRealPath(function() {
    ...
    if (cache && (client.startTime < cache.expireTime)) {
      ...
      if (client.res.websocket && application.websocket) application.websocket.initialize(client);
      if (client.res.websocket && application.rpc) application.rpc.initialize(client);
  ...
  client.fileHandler('error', false, function() {
    if (code === 304) client.end(); else {
      if (client.typeExt === 'json') client.end('{"statusCode":' + code + '}');
        ...
        client.include({ title: 'Error ' + code, message: message }, application.systemTemplates['error'], '', function(tpl) {
  ...
  if (client.path === '/') callback();
    ...
    if (buffer) {
      ...
      if (err) {
        ...
        else if (stats.isDirectory()) client.index(filePath);
