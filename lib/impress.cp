(function(impress) {
		...
		impress.getUser(login, function(err, node) {
			if (!node) {
				db.impress.users.insert({
					...
					impress.sendPassword(login);
					if (callback) callback(null, nodes[0]);
				});
			} else if (callback) callback("already registered", node);
		});
	}
	...
	function sse(req, res) {
		var userId = req.impress.session ? impress.sessions[req.impress.session].userId.toHexString() : null;
	...
	function execute(req, res, filePath, callback) {
		...
		req.impress.access.allowed = (
			(
				...
				(!!req.impress.session && req.impress.access.logged)
			) && (
	...
	impress.template = function(req, res, data, file, cursor, callback) { // callback(tpl)
		var userGroup = '';
		if (req.impress.session) userGroup = '.'+(req.impress.user.group || 'everyone');
