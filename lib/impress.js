(function(impress) {
	...
	function processing(req, res) {	
		...
		// Add connection to SSE pool
		if (req.impress.ext == 'sse') {
			if (req.impress.session) {
				...
				if (!impress.sessions[req.impress.session].sse) impress.sessions[req.impress.session].sse = [];
				impress.sessions[req.impress.session].sse.push({ request: req, response: res });
			...
			req.impress.access.allowed = req.impress.access.allowed && (
				...
				(req.impress.access.groups &&
					(
						...
						inArray(req.impress.access.groups, impress.sessions[req.impress.session].group)
					)
				)
			);
		}
