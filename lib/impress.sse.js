...
impress.sse.mixinApplication = function(application) {
  ...
  application.sse.heartbeat = setInterval(function() {
    var connection, connectionId,
        connections = application.sse.connections;
    for (connectionId in connections) {
      connection = connections[connectionId];
      if (connection.heartbeat) connection.res.write(SSE_INIT_BUF);
    }
  }, 10000);
  ...
  application.sse.sendToUser = function(userId, eventName, data) {
    ...
    var key;
    if (user && user.sse) for (key in user.sse) user.sse[key].res.write(buf);
  };
  ...
  application.sse.sendToChannel = function(channel, eventName, data) {
    var buf = impress.sse.packet(eventName, data),
        i, j, userId, user, userIds = application.sse.channels[channel];
    for (j = 0; j < userIds.length; j++) {
      userId = userIds[j];
      user = application.users[userId];
      if (user && user.sse) {
        for (i = 0; i < user.sse.length; i++) user.sse[i].res.write(buf);
      }
    }
  };
