...
Client.prototype.attachment = function(attachmentName, size, lastModified) {
  ...
  var fileName = 'attachment; filename="' + attachmentName + '"';
...
Client.prototype.uploadFile = function(file, onFile) {
  ...
  if (isComp || isNotComp) {
    ...
    api.mkdirp(targetDir, function(/*err*/) {
      var ws = api.fs.createWriteStream(data.storagePath);
      var rs = api.fs.createReadStream(tempFile);
      rs.pipe(ws);
...
Client.prototype.stream = function(filePath, stats) {
  ...
  if (range) {
    ...
    var cRange = 'bytes ' + start + '-' + end + '/' + stats.size;
