...
impress.mixinApplication = function(application) {
  ...
  application.addToCache = function(filePath, stats, compressed, data) {
    if (data) {
      application.cache.static[application.relative(filePath)] = { data: data, stats: stats, compressed: compressed };
  ...
  application.updateCache = function() {
    ...
    api.async.each(updates, function(relPath, callback) {
      ...
      //console.log('updateCache try ' + application.dir + '+' + relPath);
      api.fs.exists(filePath, function(exists) {
        if (exists) {
          api.fs.stat(filePath, function(err, stats) {
  ...
  application.watchCache = function(relPath) {
    //if (relPath === '//app') console.trace('!!!');
    //console.log('watchCache relPath=' + relPath);
    var filePath = application.dir + relPath,
        watchInterval = api.impress.getByPath(impress.config, 'scale.watchInterval') || 2000,
        watcher, path,
        char = filePath.slice(-1);
    //console.log('watchCache filePath=' + filePath);
    //if (char === '/' || char === '\\') path = filePath;
    //else path = api.impress.stripTrailingSlash(api.path.dirname(relPath));
    path = filePath;
    if (application) {
      ...
      if (!watcher) {
        api.fs.exists(path, function(exists) {
          if (exists) {
            //console.log('watchCache path=' + path);
            watcher = api.fs.watch(path, function(event, fileName) {
              ...
              watcher = application.cache.watchers[relPath];
              if (application.cache.timer) clearTimeout(application.cache.timer);
              //console.log('changed fileName='+fileName+' relPath='+relPath+' path='+path);
