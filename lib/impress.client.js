          ...
          api.mkdirp(targetDir, function(err) {
            ...
            fd.on('end', function() {
              ...
              if (data.compressionFlag === 'Z') {
                var zip = new api.zipstream(),
                    out = api.fs.createWriteStream(data.storagePath + '.tmp');
                zip.pipe(out);
                zip.on('end', function() { saveFile(data, onFile); });
                zip.entry(api.fs.createReadStream(data.storagePath), { name: data.originalName }, function(err, entry) {
                  if (err) throw err;
                  zip.finalize();
                });
              } else if (data.compressionFlag === 'G') {
                var gzip = api.zlib.createGzip(),
