...
//
impress.fork = function(env, inspect) {
...
impress.compareHosts = function() {
  var i, j, k, ilen, jlen, klen, appName,
      config, hosts, cmp = [];
  for (appName in impress.applications) {
...
impress.detectAppDir = function(appName, callback) {
  ...
  var linkFile = dir + '/application.link';
...
impress.linkNamespaces = function() {
  var appName, application, exp, imp, impHash,
...
//
impress.startServer = function(server) {
...
//
impress.logServerStarted = function(server) {
  var protocolName = server.protocol.toUpperCase();
...
//   server - server configuration object
//
impress.configureServer = function(server) {
...
//
impress.forkWorker = function(workerId, serverName, inspect) {
  var worker, env = {};
  ...
  worker.on('exit', function(code /*signal*/) {
    ...
    if (code > 0) api.timers.setImmediate(function() {
...
impress.forkLongWorker = function(appName, workerFile, clientData) {
  ...
  if (application) {
    if (process.isMaster) {
      var env = {};
      var workerId = impress.nextWorkerId;
...
impress.dispatcher = function(req, res) {
  ...
  var application, client, appName, appFound,
      host = api.common.parseHost(req.headers.host);
  for (appName in impress.applications) {
    application = impress.applications[appName];
    if (application) {
      appFound = (application.config.hosts && (
        (application.config.hosts.indexOf(host) > -1) ||
