...
Client.prototype.startSession = function() {
  ...
  if (application && !client.session) {
    client.sid = impress.generateSID(application.config);
    client.user = {};
    client.session = {};
...
Client.prototype.destroySession = function() {
  ...
  if (application && client.session) {
    ...
    if (userId && application.users[userId]) delete application.users[userId].sessions[client.sid];
    delete application.sessions[client.sid];
    // TODO: delete session from DB persistent session storage
    if (impress.security) impress.security.deletePersistentSession(client.sid);
    client.sid = null;
    client.user = null;
    client.session = null;
  }
              ...
              api.async.each(flist, function(fileName, cb) {
                ...
                api.async.parallel({
                  stats: function(callback) {
                    api.fs.stat(filePath, callback);
                  },
                  get: function(callback) {
                    api.fs.exists(filePath + '/get.js', callback);
                  },
                  post: function(callback) {
                    api.fs.exists(filePath + '/post.js', callback);
                  }
