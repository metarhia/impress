...
class Application extends api.events.EventEmitter {
  ...
  constructor(name, dir) {
    ...
    impress.createSandbox(this, () => {
      ...
      this.config.loadConfig(() => {
        this.workerId = impress.workerId;
  ...
  // Load application library from `path`
  ...
  // Load application api from `api/` directory
  ...
  loadPlaceFile(placeName, path, file, callback) {
    ...
    impress.createScript(this, fileName, (err, exports) => {
      ...
      if (placeName === 'tasks') {
        ...
        const doneFileName = api.path.format({
          dir: path,
          name: sectionName,
          ext: '.done',
        });
