...
var Client = function(application, req, res) {
  ...
  this.req = req;
  this.res = res;
  this.socket = req.socket || req.connection.socket;
  ...
  if (res.setHeader) {
    ...
    res.setHeader('Keep-Alive', 'timeout=' + ~~(server.listener.keepAliveTimeout / 1000));
  }

  res.on('finish', function resOnFinish() {
    var req = socket.parser && socket.parser.incoming,
        res = socket._httpMessage;
    socket.removeAllListeners('timeout');
    if (req) req.removeAllListeners('timeout');
    if (res) res.removeAllListeners('timeout');
    socket.setTimeout(self.keepAliveTimeout || impress.DEFAULT_KEEP_ALIVE_INTERVAL, impress.serverOnTimeout);
  });
...
Client.prototype.end = function(output) {
  ...
  if (!client.finished) {
    ...
    if (socket) delete socket.client;
  }
