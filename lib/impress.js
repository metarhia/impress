...
impress.forkLongWorker = function(appName, workerFile, clientData) {
  ...
  if (application) {
    if (api.cluster.isMaster) {
      ...
      worker.nodeId = impress.config.scale.cluster+'L'+worker.id;
      impress.listenWorker(worker);
  ...
  worker.on('message', function(message) {
    ...
    if (application) {
      if (message.name === 'impress:event') { // propagate to all workers except of original sender
        ...
        var worker = impress.forkLongWorker(message.appName, message.workerFile, message.clientData);
        message.pid = worker.process.pid;
        message.nodeId = worker.nodeId;
