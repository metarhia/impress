...
class Application extends api.events.EventEmitter {
  ...
  loadSchemas(callback) {
    ...
    impress.createScript(this, loaderPath, (err, getMsConfig) => {
      ...
      const { options, config } = getMsConfig();
      options.context = this.sandbox;

      const loadSchema = api.util.callbackify(api.metaschema.fs.load);
      loadSchema(dir, options, config, (err, result) => {
        if (err) {
          this.log.error(
            `Schema loading error: ${err.message} at ${err.stack}`
          );
          callback(err);
          return;
        }
        const [errors, schema] = result;
        if (errors.length > 0) {
          const msg = `Schema processing error:\n${errors.join(',\n')}`;
