...
impress.security.mixin = function(application) {
  ...
  // Session class
  //   session - an object from storage provider
  //     we need to copy all parameters from
  //
  application.security.Session = function(session) {
    this.category = session.category;
    this.id = session.id;
    this.login = session.login;
    this.sid = session.sid || api.common.generateSID(application.config);
      ...
      application.security.getUser(login, function(err, user) {
        if (!user) {
          ...
          application.databases.security.create(record, function(err) {
            var user;
            if (!err) {
              user = new application.security.User(record);
            }
            callback(err, user);
          });
        } else {
  ...
  application.security.getUser = function(login, callback) {
    if (application.security.hasDb()) {
      ...
      .fetch(function(err, user) {
        if (user) callback(null, new application.security.User(user));
        else callback(err);
      });
          ...
          if (login) {
            if (application.users[login]) {
              ...
              application.security.getUser(login, function(err, item) {
                if (item) {
                  var user = new application.security.User(item);
