...
Client.prototype.uploadFile = function(file, onFile) {
  ...
  var folder1 = api.common.generateKey(2, impress.DIGIT),
  ...
  var tempFile = file.path,
      ...
      isCompressed = api.common.inArray(application.extCompressed, fileExt),
      isNotCompressed = api.common.inArray(application.extNotCompressed, fileExt);
...
Client.prototype.stream = function(filePath, stats) {
  ...
  if (range) {
    ...
    var allowOrigin = api.common.getByPath(application.config, 'application.allowOrigin');
    if (allowOrigin) {
