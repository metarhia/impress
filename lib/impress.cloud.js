  ...
  impress.cloud.init = function() {
    if (api.cluster.isMaster && impress.config.cloud) {
      ...
      if (cc.type === 'controller') {
        ...
        impress.cloud.pub.bind(cc.controller+':'+cc.pubSubPort, function(err) {
          if (err) console.log(err);
          else console.log('  zmq publisher '+cc.pubSubPort);
        });
        ...
        impress.cloud.rep.bind(cc.controller+':'+cc.reqResPort+'', function(err) {
          if (err) console.log(err);
          else console.log('  zmq reply '+cc.reqResPort);
        });
        ...
        impress.cloud.sub.connect(cc.controller+':'+cc.pubSubPort);
        impress.cloud.sub.subscribe('');
        console.log('  zmq subscriber '+cc.pubSubPort);
        ...
        impress.cloud.req.connect(cc.controller+':'+cc.reqResPort);
        console.log('  zmq request '+cc.reqResPort);
