...
//  return (typeof(fn) === 'function') ? fn : api.common.emptyness;
//};
...
// Iterate items array in parallel
...
// Iterate items array in series
//   items - array of items to be processed
//   iterator(item, callback) - functionton be called
//   done - done callback
//
...
// Async execute  array of functions in series
...
//   done - done callback
//
api.common.parallel = function(funcs, done) {
  var i = -1,
      len = funcs.length;
  //done = api.common.safeFunc(done);
  function next() {
...
// Async filter items in series (analogue of filterSeries)
//   items - array of items to be processed
//   iterator(item, callback) - functionton be called
//     callback(bool) - iterator should return true to accept item and false to reject
//   done - done callback
//
api.common.filterAsync = function(items, iterator, done) {
  var i = -1,
      len = items.length,
      filtered = [];
  //done = api.common.safeFunc(done);
  function next() {
    i++;
    iterator(items[i], function(result) {
      if (result) filtered.push(items[i]);
      if (i < len - 1) next();
      else done(filtered);
    });
  }
  if (len < 1) done(filtered);
  else next();
};

// Async detect items in series
//   items - array of items to be processed
//   iterator(item, callback) - functionton be called
//     callback(bool) - iterator should return true to accept item and false to reject
//   done - done callback
//
api.common.detectAsync = function(items, iterator, done) {
  var i = -1,
      len = items.length;
  //done = api.common.safeFunc(done);
  function next() {
