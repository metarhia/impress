  ...
  application.createScript(fileName, (err, fn) => {
    if (err) {
      ...
      if (handler === 'lazy') {
        ...
        cb();
      } else client.executeFunction(fn, callback);
    } else callback();
  });
};
...
Client.prototype.executeFunction = function(
  ...
  if (typeof(fn) === 'function') {
    ...
    if (fn.length === 2) {
      fn(client, (result, errorCode, headers) => { // Execute Impress handlers
...
Client.prototype.serveStatic = function(
  ...
  function serveFile(err, stats) {
    if (err) {
      //application.cache.static.add(relPath, impress.FILE_NOT_FOUND);
      application.cache.watch(api.path.dirname(relPath));
