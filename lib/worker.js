...
const { node: { worker, fsp, path } } = require('./dependencies.js');
...
(async () => {
  ...
  const logger = await metalog({
    path: logPath, workerId: threadId, ...config.sections.log, home
  });
  ...
  const ports = config.sections.server.ports.length + balancer;
  if (threadId <= ports) {
