    ...
    if (mu.heapTotal > impress.memoryLimit) {
      ...
      if (mu.heapTotal > impress.memoryLimit) {
        impress.fatalError(
          ...
          api.impress.bytesToSize(mu.heapTotal) +
          ', restarting'
        );
      }
    }
        ...
        if (impApp && impHash && exp) {
          for (expObjName in impHash) {
            ...
            if (exp.indexOf(expObjName) !== -1) {
              api.impress.setByPath(application.sandbox, impObjName, impObj);
            } else {
    ...
    if (master) {
      if (single) {
        if (server.protocol === 'https') {
          server.listener = api.https.createServer(
            impress.loadCertificates(server),
            impress.dispatcher
          );
        } else {
          server.listener = api.http.createServer(impress.dispatcher);
        }
        if (impress.websocket) {
          impress.websocket.upgradeServer(server.listener);
        }
      } else if (sticky) {
        server.listener = api.net.createServer(impress.balancer);
      } else if (cluster) {
        server.listener = {
          close: api.impress.emptyness,
          on: api.impress.emptyness,
          listen: api.impress.emptyness
        };
    ...
    if (server.listener) {
      ...
      if ((master && !bundle) || (!master && !cloned)) {
        if (server.nagle === false) {
          server.listener.on('connection', impress.serverSetNoDelay);
        }
        if (server.address === '*') {
...
impress.balancer = function(socket /* res */) {
  ...
  if (impress.config.scale.strategy === 'sticky') {
    ip = api.impress.ip2int(socket.remoteAddress);
