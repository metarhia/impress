(function(impress) {
	...
	impress.sse.connect = function(client) {
		var userId = (client.session && client.logged)
			? impress.sessions[client.session].userId.toHexString() : null;
		...
		if (client.sse && client.sse.channel) {
			...
			if (userId) {
				...
				impress.users[userId].sse[connectionId] = client;
			}
	...
	impress.sse.sendToUser = function(userId, eventName, data, isRetranslation) {
		...
		if (impress.users[userId] && impress.users[userId].sse) {
			for (var i in impress.users[userId].sse) impress.users[userId].sse[i].res.write(buf);
		}
	}
