...
const { findHandler } = require('./scheduler.js');
...
(async () => {
  ...
  parentPort.on('message', async (data) => {
    ...
    if (data.type === 'invoke') {
      const handler = findHandler(application.sandbox, data.target);
      if (!handler) {
        data.port({ error: new Error('Handler not found') });
        return;
      }
      try {
        const result = await handler(data.args);
        data.port.postMessage({ data: result });
      } catch (err) {
        data.port.postMessage({ error: err });
