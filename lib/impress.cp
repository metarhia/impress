(function(impress) {
	...
	impress.startSession = function(req, res) {
		if (!req.impress.session) {
			...
			if (impress.config.cluster.cookie)
				impress.setCookie(req, res, impress.config.cluster.cookie, impress.nodeId);
	...
	//   callback(isSuccess)
	//
	impress.signIn = function(req, res, callback) {
	...
	//
	impress.signOut = function(req, res, callback) {
		if (req.impress.session) {
			var userId = impress.sessions[req.impress.session].userId;
			if (userId && impress.users[userId] && impress.users[userId].sessions)
				delete impress.users[userId].sessions[req.impress.session];
			delete impress.sessions[req.impress.session].userId;
			req.impress.sessionModified = true;
	...
	//   callback(user)
	//
	impress.register = function(req, res, callback) {
		impress.getUser(req.post.Email, function(err, user) {
			if (!user) {
				db.impress.users.insert({
					...
					var user = users[0];
					if (user) {
						impress.sendPassword(req.post.Email);
						impress.startSession(req, res);
			...
			} else if (callback) callback("already registered", user);
		});
	}
	...
	// Get user record from database
	// 
	impress.getUserById = function(userId, callback) {
		...
		if (sid) {
			if (impress.validateSID(sid)) {
				if (impress.sessions[sid]) {
					...
					req.impress.user = getSessionUser(sid);
					processing(req, res);
				} else {
							...
							if (session) {
								...
								if (userId) {
									if (impress.users[userId]) {
										if (!impress.users[userId].sessions) impress.users[userId].sessions = [];
										impress.users[userId].sessions.push(req.impress.session);
										req.impress.logged = true;
										processing(req, res);
									} else {
										impress.getUserById(userId, function(err, node) {
											if (node) {
												var user = impress.user(node);
												user.sessions.push(req.impress.session);
												impress.users[userId] = user;
												req.impress.logged = true;
												req.impress.user = user;
											} else {
	...
	function processing(req, res) {	
		...
		async.eachSeries(req.impress.handlers, function(handler, callback) {
			req.impress.path = req.impress.url;
			filehandler(req, res, handler, function() {
				callback();
			});
		}, function(err) {
			req.impress.path = req.impress.url;
			if (req.impress.access.allowed) {
				if (req.impress.ext == 'html') {
					...
					if (req.impress.ext == 'sse') sse(req, res);
					else {
						var output = '';
	...
	function baseHeader(ext, stats) {
		...
		if (!inArray(impress.compressedExt, ext) && stats.size>impress.compressAbove) header['Content-encoding'] = 'gzip';
		if (stats) {
