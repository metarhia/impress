...
//
api.jstp.PACKET_DELIMITER = PACKET_DELIMITER;

// Chunks is an Array of Buffer
//
api.jstp.Chunks = function() {
  this.items = [CHUNKS_FIRST];
};
...
api.jstp.Chunks.prototype.add = function(chunk) {
  var startPos = chunk.length - DELIMITER_LENGTH,
...
api.jstp.connect = function(name, host, port) {
  ...
  connection.call = function(interfaceName, methodName, parameters, callback) {
    connection.send(
      api.jstp.packet(
        'call', connection.packetId++, interfaceName, methodName, parameters
      )
    );
  };
  ...
  connection.event = function(interfaceName, eventName, parameters) {
    connection.send(
      api.jstp.packet(
        'event', connection.packetId++, interfaceName, eventName, parameters
      )
    );
  };
