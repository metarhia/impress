...
class Client {
  ...
  constructor(application, req, res) {
    ...
    const server = impress.config.sections.servers[socket.server.serverName];
  ...
  runScript(handler, fileName, callback) {
    ...
    impress.createScript(application, fileName, (err, fn) => {
      if (err) {
        this.err = err;
        callback();
        return;
  ...
  upload(each, callback) {
    ...
    for (const fieldName in this.files) {
      ...
      for (const key in field) {
        ...
        fileCount++;
        impress.uploadFile(application, file, cb);
      }
    }
  }
