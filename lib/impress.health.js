(function(impress) {
	...
	impress.health.monitoringInterval = duration(
		(impress.config.health && impress.config.health.monitoringInterval)
	...
	impress.health.init = function() {
		if (impress.cluster.isMaster) {
			clearDataStructures();
			if (impress.cluster.strategy == "single") {
	...
	impress.health.getClusterState = function() {
		...
		return {
			...
			load5:   load[1],
			load15:  load[2],
			spawns:  impress.stat.workersRestart,
			sse:     impress.stat.sseRetranslate
		};
	}
