...
api.definition.require = function(fileName) {
  if (!api.impress.contains(fileName, '/')) fileName = '../schemas/' + fileName;
  var definition = null;
  try {
...
api.definition.walkRecursive = function(parent, data, definition, name, path, callback) {
  ...
  if (typeof(tpl) === 'string') {
    ...
    api.impress.merge(dataKeys, tplKeys);
...
var definitionRx = new RegExp('[' + api.impress.escapeRegExp('[]{}()') + ']', 'g');
...
api.definition.parse = function(tpl) {
  ...
  var commentPos = tpl.indexOf('//'),
      comment = '';
  if (commentPos !== -1) {
    comment = api.impress.trim(tpl.substring());
    tpl = api.impress.trim(tpl.substring(0, commentPos));
  }
  if (typeof(tpl) === 'object') return { type: 'hash', str: tpl };
  var def = { type:'', optional: api.impress.contains(tpl, '[') || api.impress.contains(tpl, ':') },
      name = tpl.replace(definitionRx, '');
  if (api.impress.contains(tpl, '{{')) def.type = 'collection';
  if (def.type !== 'collection' && api.impress.contains(tpl, '{')) def.type = 'struct';
  if (api.impress.contains(tpl, '(')) def.type = 'set';
  if (api.impress.contains(name, ':')) {
...
api.definition.preprocessValue = function(parent, data, definition, name, path, def) {
  if (def.type === 'size') parent[name] = api.impress.sizeToBytes(data);
  else if (def.type === 'duration') parent[name] = api.impress.duration(data);
...
api.definition.validate = function(data, definition, name, preprocess) {
  ...
  api.definition.walk(data, definition, name, function(parent, data, definition, name, path, def) {
    ...
    if (def.type !== 'undefined') {
      ...
      else if (def.type === 'set')        itemValid = api.impress.inArray(def.variants, data);
      ...
      else if (def.type === 'size')       itemValid = api.impress.inArray([ 'string', 'number' ], dataType);
      else if (def.type === 'duration')   itemValid = api.impress.inArray([ 'string', 'number' ], dataType);
      else if (def.type === 'struct')     itemValid = dataType === 'object';
      else if (def.type === 'hash')       itemValid = dataType === 'object';
      else if (def.type === 'collection') itemValid = api.impress.inArray([ 'array', 'object' ], typeof(parent));
