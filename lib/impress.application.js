...
impress.mixinApplication = function(application, callback) {
  ...
  var domain = api.domain.create();

  // Create execution domain
  //
  application.catchException = function(err) {
    if (domain.client) {
      domain.client.error(500, err);
      domain.client = null;
    }
    application.logException(err);
  };

  domain.on('error', application.catchException);
  ...
  application.preloadDirectory = function(relPath, depth, callback) {
    ...
    api.fs.readdir(absPath, function(err, files) {
      if (err) {
        ...
        application.watchCache(trailingSlash(absPath));
  ...
  application.loadPlaceScripts = function(placeName, callback) {
    ...
    api.fs.exists(placeDir, function(exists) {
      if (exists) {
        ...
        if (!inArray([ 'setup', 'init' ], placeName)) application.watchCache(trailingSlash(placeDir));
      } else if (callback) callback();
    });
  };
  ...
  // Attach handler before
  //   handler - impress 2 parameter functon or 3 parameter middleware
  //
  application.before = function(handler) {
    application.before.handlers.push(handler);
  };
  // Array of before handlers
  application.before.handlers = [];
  // Execute all before hendlers
  application.before.emit = function(client, callback) {
    api.async.eachSeries(application.before.handlers, function(fn, cb) {
      fn(client, cb);
    }, function() {
      if (callback) callback();
    });
  ...
  // Attach handler after
  //   handler - impress 2 parameter functon or 3 parameter middleware
  //
  application.after = function(handler) {
    application.after.handlers.push(handler);
  };
  // Array of after handlers
  application.after.handlers = [];
  // Execute all after hendlers
  application.after.emit = function(client, callback) {
    api.async.eachSeries(application.after.handlers, function(fn, cb) {
      fn(client, cb);
    }, function() {
      if (callback) callback();
    });
        ...
        if (exists) {
          api.fs.stat(filePath, function(err, stats) {
            ...
            if (stats.isFile()) files[filePath] = stats;
              ...
              var key;
              for (key in application.cache.files) {
  ...
  application.watchCache = function(filePath) {
    ...
    path = stripTrailingSlash(api.path.dirname(path));
    if (application) {
        ...
        api.fs.exists(path, function(exists) {
          if (exists) {
            watcher = api.fs.watch(path, function(event, fileName) {
              ...
              if (fileName) filePath = path + '/' + stripTrailingSlash(fileName);
              else filePath = path;
              watcher = application.cache.watchers[path];
              if (application.cache.timer) clearTimeout(application.cache.timer);
