(function(impress) {
	...
	if (impress.config.plugins && impress.config.plugins.require) {
		if (impress.cluster.isMaster && impress.config.plugins.require.indexOf("impress.log") == -1) {
			console.log('Warning: plugin impress.log.js is not included into config require section'.red);
		}
	...
	impress.init = function(callbacks) {
		...
		function startup(callback) {
			...
			if (impress.cluster.isMaster) {
				process.title = "Impress Application Server: master process";

				impress.workersRestart = 0;
				impress.workers = [];
				...
				process.title = "Impress Application Server: worker process";
				...
				process.on('message', function(message, socket) {
					...
					if (message.name == 'impress:socket') {
						...
						if (message.user) impress.sse.userEvent(message.user, message.event, message.data, true);
						else if (message.channel) impress.sse.channelEvent(message.channel, message.event, message.data, true);
						else if (message.global) impress.sse.globalEvent(message.event, message.data, true);
					}
				});
			}
	...
	impress.spawn = function(workerId, serverName) {
		...
		worker.on('message', function(msg) {
			if (msg.name == 'impress:sse') { // propagate to all workers except of original sender
				impress.stat.sseRetranslate++;
				for (var id = 0; id < impress.workers.length; id++) {
					if (id != workerId) impress.workers[id].send(msg);
				}
			}
		});
	}
	...
	function dispatcher(req, res) {
		...
		req.impress.slowTime = server.slowTime || impress.defaultSlowTime;
	...
	//
	impress.sse.globalEvent = function(eventName, data, isRetranslation) {
		var packet = 'event: '+eventName+'\ndata: '+JSON.stringify(data)+'\n\n',
			buf = new Buffer(packet, 'utf8'),
			isRetranslation = isRetranslation || false;
