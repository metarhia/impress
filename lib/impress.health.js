...
impress.health.init = function() {
  if (impress.config.scale.health) {
    impress.health.timer = setInterval(function() {
      impress.log.health(impress.health.getProcessState());
...
//
impress.health.getProcessState = function() {
  var cpu = impress.health.cpuTimes(),
      load = api.os.loadavg(),
      memoryUsage = process.memoryUsage();
  return ([
    Math.round(api.os.uptime()), api.os.freemem(),
    memoryUsage.rss, memoryUsage.heapTotal, memoryUsage.heapUsed,
...
impress.health.getClusterState = function() {
  ...
  return {
    ...
    load5:   load[1],
    load15:  load[2],
    forks:   api.cluster.workers ? Object.keys(api.cluster.workers).length : 0,
