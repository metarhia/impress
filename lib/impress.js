    ...
    if (master) {
      if (single) {
        if (server.protocol === 'jstp') {
          server.instance = api.net.createServer(impress.dispatcher.jstp);
        } else {
          if (server.protocol === 'https') {
            server.instance = api.https.createServer(
              impress.loadCertificates(server),
              impress.dispatcher.http
            );
          } else if (server.protocol === 'http') {
            server.instance = api.http.createServer(impress.dispatcher.http);
          }
...
//
impress.dispatcher.http = function(req, res) {
...
//
impress.dispatcher.jstp = function(socket) {
  var chunks = [];
  socket.on('data', function(data) {
    chunks.push(data);
    if (data.indexOf('}') > -1) {
      var application, key, val,
          packet = Buffer.concat(chunks).toString(),
          handshake = api.jstp.parse(packet);
      console.dir(handshake);
      for (key in handshake) {
        val = handshake[key];
        if (key === 'handshake') {
          application = impress.applications[val[1]];
        } else {
          console.dir({sid:key,pwd:val[0]});
        }
      }
      if (application) {
        console.log('Selected app: ' + application.name);
        var connection = new application.Connection(appication, socket);
        application.connections.push(connection);
      } else {
        socket.end('{handshake:[-1],error:[4]}');
      }
    }
  });
};
