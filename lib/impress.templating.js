...
Client.prototype.template = function(
  ...
  api.metasync.find(files, (item, cb) => {
    ...
    else api.fs.access(filePath, err => cb(!err));
  }, (result) => {
    if (fileCache) {
...
Client.prototype.include = function(
  ...
  api.fs.readFile(filePath, 'utf8', (err, tpl) => {
    if (err) return callback(impress.TPL_NOT_FOUND + filePath);
    if (!tpl) {
...
Client.prototype.parseTemplate = function(
  ...
  const doc = [];
  let pos, tplInclude, dataInclude, tplBody, arrayIndex, name;
...
Client.prototype.render = function(
  ...
  if (tpl === impress.FILE_IS_EMPTY) return callback('');
