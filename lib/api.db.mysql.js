...
if (api.mysql) {
  ...
  api.db.mysql.schema.generateScript = function(databaseSchema, consoleOutput) {
    var validationResult = api.db.schema.validate(
    ...
    if (validationResult.valid) {
      ...
      var table, tableName, primaryKey, primaryFieldDef,
      ...
      for (tableName in databaseSchema) {
        if (tableName !== 'caption' && tableName !== 'version') {
          ...
          for (fieldName in table.fields) {
            ...
            if (
              field.primary ||
              (fieldIndex === 1 && api.common.inArray(['id', 'uid'], field.type))
            ) {
            ...
            if (
              ...
              referenceStatement = (
                altr + tableName + ' ADD CONSTRAINT fk' + tableName + fieldName +
          ...
          if (references.indexes) {
            for (indexName in references.indexes) {
              ...
              sqlScript += (
                ...
                'INDEX ' + (compositeIndex.unique ? 'ak' : 'idx') + indexName +
  ...
  api.db.mysql.schema.executeScript = function(target, script, callback) {
    var connection = api.db.drivers.mysql.createConnection(
