...
if (zmq) {
  ...
  api.zmq = zmq;
  impress.cloud.instance = 'standalone'; // standalone, controller, server, balancer, brocker
  ...
  impress.cloud.init = function() {
    if (api.cluster.isMaster && impress.config.scale) {
      var cc = impress.config.scale;
      if (cc.instance === 'controller') {
        ...
        impress.cloud.pub.bind(cc.controller+':'+cc.subPort, function(err) {
          if (err) console.log(err);
          else console.log('  zmq publisher '+cc.subPort);
        });
        ...
        impress.cloud.rep.bind(cc.controller+':'+cc.reqPort+'', function(err) {
          if (err) console.log(err);
          else console.log('  zmq reply '+cc.reqPort);
        });
        ...
        impress.cloud.sub.connect(cc.controller+':'+cc.subPort);
        impress.cloud.sub.subscribe('');
        console.log('  zmq subscriber '+cc.subPort);
        ...
        impress.cloud.req.connect(cc.controller+':'+cc.reqPort);
        console.log('  zmq request '+cc.reqPort);
