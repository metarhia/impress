(function(impress) {
	...
	impress.log.debug = function(message) {
		impress.log.write(impress.log.fdDebug, message);
	}

	// Write error to log
	//
	impress.log.slow = function(message) {
		impress.log.write(impress.log.fdSlow, message);
	}
	...
	//
	impress.init = function(cbInstance, cbMasterInstance, cbWorkerInstance) {
		...
		function startup(callback) {
			...
			impress.mkdirp(impress.dir+'/log', function(err) {
				if (err) console.error(err);
					...
					if (impress.config.log.debug) impress.log.fdDebug  = impress.fs.createWriteStream(
						impress.dir+'/log/'+impress.config.log.debug, {flags: 'a'}
					);
					if (impress.config.log.slow) impress.log.fdSlow  = impress.fs.createWriteStream(
						impress.dir+'/log/'+impress.config.log.slow, {flags: 'a'}
					);
				}
			});
