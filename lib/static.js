...
class Static extends Place {
  ...
  constructor(name, application, options = {}) {
    ...
    if (compressType) {
      ...
      this.compressorFactory = compressor;
    }
  }
  ...
  compress(filePath) {
    ...
    return node.streamConsumers.buffer(compressor);
  }
  ...
  async serve(url, req, res) {
    ...
    const absPath = path.posix.join(this.path, filePath);
