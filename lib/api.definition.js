...
api.definition.require = function(fileName) {
  ...
  if (!api.common.contains(fileName, '.js')) {
    fileName += '.js';
  }
  var configFile = api.fs.readFileSync(fileName),
...
// Internal walk
//
api.definition.walkRecursive = function(
  parent, data, definition, name, path, callback
) {
  ...
  if (typeof(tpl) === 'string') {
    ...
    for (key in data) {
      ...
      api.definition.walkRecursive(
        item, item, definition, def.name, itemPath, callback
      );
    }
  } else {
    ...
    for (var i = 0, len = dataKeys.length; i < len; i++) {
      ...
      if (item) {
        if (def.type === 'struct') {
          api.definition.walkRecursive(
            data, item, definition, def.name, itemPath, callback
          );
          ...
          if (other) {
            ...
            for (subKey in item) {
              ...
              api.definition.walkRecursive(
                item, subItem, definition, def.name, subItemPath, callback
              );
            }
          }
        }
...
api.definition.parse = function(tpl) {
  ...
  if (commentPos !== -1) {
    // var comment = api.common.trim(tpl.substring());
    tpl = api.common.trim(tpl.substring(0, commentPos));
  }
  if (typeof(tpl) === 'object') return { type: 'hash', str: tpl };
  var def = {
    type: '',
    optional: api.common.contains(tpl, '[') || api.common.contains(tpl, ':')
  };
  var name = tpl.replace(definitionRx, '');
  if (api.common.contains(tpl, '{{')) def.type = 'collection';
  if (def.type !== 'collection' && api.common.contains(tpl, '{')) {
    def.type = 'struct';
  }
...
//
api.definition.preprocessValue = function(
  parent, data, definition, name, path, def
) {
  if (def.type === 'size') {
