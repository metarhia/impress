...
db.openApplicationDatabases = function(application, callback) {
  ...
  if (databases && Object.keys(databases).length > 0) {
    api.impress.each(Object.keys(databases), function(databaseName, cb) {
...
db.openDatabase = function(application, database, callback) {
  ...
  database.slowTime = api.impress.duration(database.slowTime || DEFAULT_SLOW_TIME);
  database.retryCount = database.retryCount || DEFAULT_RETRY_COUNT;
  database.retryCounter = 0;
  database.retryInterval = api.impress.duration(database.retryInterval || DEFAULT_RETRY_INTERVAL);
