...
if (sharedCache && sharedCache.segments) {
  // Limit mode: build segmentsMap, import LimitCache for projection
  ...
  // Per-file mode: no segments, import PerFileCache for projection
  const { PerFileCache } = require('./cache/PerFileCache.js');
...
// Pre-initialize ACK: real function for limit mode, noop for per-file
const sendAck =
  sharedCache && sharedCache.segments
...
const handlers = {
  'file-update': (msg) => {
    ...
    for (const [key, entry] of updates) {
      projected.set(key, projectEntry(entry));
    }
