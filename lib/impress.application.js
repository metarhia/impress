...
impress.mixinApplication = function(application, callback) {
  ...
  application.preprocessConfig = function() {
    ...
    var configDefinition = (application === impress) ? impress.serverConfigDefinition : impress.applicationConfigDefinition,
        result = api.definition.validate(config, configDefinition);
    if (!result.valid) {
      console.log('Errors in configuration found. Application: '+application.name.yellow.bold);
      var errorKey;
