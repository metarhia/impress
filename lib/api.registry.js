...
api.registry.deprecated = {
  require: 'Use namespaces instead, example: api.fs.readFile()',
  clearImmediate: 'Specify namespace api.events.clearImmediate()',
  clearInterval: 'Specify namespace api.events.clearInterval()',
  clearTimeout: 'Specify namespace api.events.clearTimeout()',
  setImmediate: 'Specify namespace api.events.setImmediate()',
  setInterval: 'Specify namespace api.events.setInterval()',
  setTimeout: 'Specify namespace api.events.setTimeout()',
...
api.registry.deprecate = function(fn, msg) {
  ...
  function deprecated() {
    if (!warned) {
      //if (!!impress.log.error) impress.log.error(msg);
      //else console.log(msg);
      api.impress.logException(new Error(msg));
      warned = true;
    }
...
// Deprecate listed in api.registry.deprecated
//
api.registry.deprecate.globals = function() {
  var fn, name, msg;
  for (name in api.registry.deprecated) {
    fn = global[name];
    msg = api.registry.deprecated[name];
    global[name] = api.registry.deprecate(fn, msg);
  }
};

// Unconnect in v0.2
// api.registry.deprecate.globals();
