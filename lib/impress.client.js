...
Client.prototype.fileHandler = function(handler, inheritance, callback) {
  ...
  if (fileCache) {
    ...
    var fileOriginal, relPath;
    if (exists) {
      client.runScript(handler, filePath, callback);
      fileOriginal = client.pathDir + fileName;
      relPath = application.relative(fileOriginal);
      if (!inheritance) application.cache.files[relPath] = application.relative(filePath);
...
Client.prototype.detectRealPath = function(callback) {
  ...
  if ((folderCache && folderCache !== impress.FILE_NOT_FOUND) || client.realPath === '/') callback();
    ...
    if (exists) {
      application.cache.folders[relPath] = impress.FILE_EXISTS;
      callback();
    } else {
...
Client.prototype.template = function(data, file, cursor, callback) {
  ...
  api.async.detectSeries(files, function(item, callback) {
    ...
    if (fileCache) {
      ...
      relPath = application.relative(fileOriginal);
      application.cache.files[relPath] = application.relative(filePath);
      application.watchCache(api.path.dirname(relPath));
    } else {
      ...
      if (client.execPath !== '/' && client.execPath !== '/app' && client.execPath !== '.') {
        ...
        application.watchCache('/app' + client.execPath);
      } else {
