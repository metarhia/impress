...
impress.dir = (
  api.path.dirname(__dirname).replace(impress.BACKSLASH_REGEXP, '/')
);
...
impress.detectAppDir = (
  ...
  api.fs.stat(dir, (err, stats) => {
    if (err || !stats.isDirectory()) return callback(null);
    api.fs.access(linkFile, (err) => {
      if (err) return callback(dir);
