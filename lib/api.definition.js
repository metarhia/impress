...
function walk(parent, data, definition, name, path, callback) {
  var tpl = definition[name],
      tplKeys = typeof(tpl) === 'object' ? Object.keys(tpl) : [],
      dataKeys = typeof(data) === 'object' && data !== null ? Object.keys(data) : [];
  dataKeys.merge(tplKeys);

  var i, j, key, item, itemType, template, def, itemPath, def, subItem, subItemPath;
  for (i = 0; i < dataKeys.length; i++) {
    key = dataKeys[i];
    item = data ? data[key] : '';
    if (tpl) {
      template = tpl[key];
      if (!template) template = tpl['_other'];
    }
    itemPath = path+'.'+key;
    ...
    if (item) {
      if (def.type === 'struct' ) walk(data, item, definition, def.name, itemPath, callback);
      else if (def.type === 'collection' && Array.isArray(item) ) {
        template = tpl[def.name];
        for (j = 0; j < item.length; j++) {
          subItem = item[j];
          subItemPath = itemPath+'.'+j;
          walk(item, subItem, definition, key, subItemPath, callback);
        }
      }
    }
...
api.definition.validate = function(data, definition, name) {
  ...
  api.definition.walk(data, definition, name, function(parent, data, definition, name, path, def) {
    ...
    if (def) {
      if      (def.type === 'set')        itemValid = inArray(def.variants, data);
      ...
      else if (def.type === 'collection') itemValid = dataType === 'array';
      else {
        itemValid = false;
        result.errors.push({ path: path, error: "Unexpected value" });
      }
