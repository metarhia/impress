...
impress.health.init = function() {
  if (impress.config.scale.health) {
    impress.health.nodeTimer = api.timers.setInterval(function() {
    ...
    if (process.isMaster) {
      impress.health.getServerInfo(function(info) {
        impress.health.serverInfo = info;
      });
      impress.health.serverTimer = api.timers.setInterval(function() {
        var serverState = impress.health.getServerState();
...
impress.health.getServerState = function() {
  ...
  return [
    ...
    api.os.freemem(),
    api.impress.workers ? Object.keys(api.impress.workers).length : 0,
    api.os.cpus().length,
...
var cmdExec = function(cmd, callback) {
  var lines, line, header, col, row,
      data = [];
  api.cp.exec(cmd, function(err, res) {
...
impress.health.getDrives = function(callback) {
  ...
  if (process.isWin) {
    ...
    api.metasync.sequential([
      function(cb) {
        cmdExec(cmdDfIn, function(data) {
          results.DfIn = data;
          cb(null);
        });
      },
      function(cb) {
        cmdExec(cmdDfSz, function(data) {
          results.DfSz = data;
          cb(null);
        });
      }
