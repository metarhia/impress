...
impress.cloud.transport.createServer = function() {
  ...
  server.on('connection', function(socket) {
    socket.on('error', impress.logException);
    impress.cloud.transport.upgradeSocket(socket);
  });
  server.on('error', impress.logException);
  return server;
};
...
impress.cloud.transport.createClient = function() {
  ...
  client.on('error', function(err) {
    ...
    if (api.cluster.isMaster) console.log('  No connection to Impress Cloud Conntroller, reconnecting...');
    if (impress.cloud.status === 'online') {
      impress.cloud.status = 'connecting';
      impress.cloud.init();
    } else if (err.code === 'ECONNREFUSED') setTimeout(impress.cloud.init, 5000);
  });
  return client;
};
...
impress.cloud.transport.upgradeSocket = function(socket) {
  ...
  socket.sendPacket = function(packet) {
    //console.log('socket.sendPacket ' + impress.nodeId);
  ...
  // Recovery connection on socket close
  //
  /*socket.on('close', function(data) {
    console.log('socket.on close in ' + impress.nodeId);
