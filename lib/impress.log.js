...
impress.log.masterTypes = [
  'error', 'debug', 'server', 'node', 'cloud', 'warning'
];

impress.log.perNodeTypes = [
  'access', 'api', 'error', 'debug', 'slow', 'node', 'warning'
];
...
impress.log.mixin = (application) => {
  ...
  application.log.init = (callback) => {
    ...
    application.log.openFile = (fileType, callback) => {
      const perNode = impress.log.perNodeTypes.includes(fileType);
      const forMaster = impress.log.masterTypes.includes(fileType);
      if (process.isMaster && !forMaster) callback();
      else {
