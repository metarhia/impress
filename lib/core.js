...
impress.isMaster = api.wt.isMainThread;

if (impress.isMaster) {
  impress.isWorker = false;
  impress.workerId = 0;
} else {
  impress.isWorker = true;
  impress.workerId = parseInt(api.wt.workerData.WORKER_ID, 10);
  impress.workerType = api.wt.workerData.WORKER_TYPE;
  impress.serverName = api.wt.workerData.WORKER_SERVER_NAME;
  impress.serverProto = api.wt.workerData.WORKER_SERVER_PROTO;
  if (impress.workerType === 'long') {
    impress.workerApplications = [api.wt.workerData.WORKER_APPNAME];
    impress.workerApplicationName = api.wt.workerData.WORKER_APPNAME;
    impress.workerApplicationFile = api.wt.workerData.WORKER_FILE;
  }
}
...
const preload = () => {
  if (impress.workerType === 'long') {
    ...
    const client = api.json.parse(api.wt.workerData.WORKER_CLIENT);
...
const ipcStart = () => {
  ...
  if (impress.isMaster) {
    ...
    process.on('beforeExit', code => {
      api.wt.parentPort.postMessage({ name: 'impress:exit', code });
...
const serverOnError = (err, address) => {
  if (['EADDRINUSE', 'EACCES'].includes(err.code)) {
    ...
    if (impress.isWorker) {
      api.wt.parentPort.postMessage({ name: 'impress:exit', error: msg });
    } else {
...
impress.load = () => {
  ...
  loadApplications(() => {
    if (impress.isMaster) {
      ...
      api.wt.parentPort.postMessage({
        name: 'impress:start', id: impress.workerId
      });
