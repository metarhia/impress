...
impress.sse.mixin = (application) => {
  ...
  application.sse.heartbeat = api.timers.setInterval(() => {
    let connection, connections = application.sse.connections;
  ...
  //
  impress.Client.prototype.sseConnect = () => {
    let client = this;
    ...
    if (client.eventChannel && !client.res.headersSent) {
      ...
      let connectionId = application.sse.nextConnectionId++;
  ...
  application.sse.sendToUser = (login, eventName, data) => {
    let buf = impress.sse.packet(eventName, data),
        user = application.users[login],
        res;
    if (user && user.sse) {
      for (let key in user.sse) {
