...
Client.prototype.startSession = function() {
  ...
  if (application && !client.session) {
    ...
    // !!! Refactor: change to new application.User, etc.
    client.user = {};
    client.session = {};
...
Client.prototype.destroySession = function() {
  // !!! Refactor: call application.security not global impress.security
  ...
  if (application && client.session) {
    ...
    if (impress.security) application.security.deletePersistentSession(client.sid);
    ...
    if (sid) {
      if (api.impress.validateSID(application.config, sid)) {
        if (impress.security && application.sessions[sid]) {
          ...
          if (impress.security) client.user = application.security.getSessionUser(sid);
          callback();
        } else {
...
Client.prototype.saveSession = function(callback) {
  if (this.application && this.session && impress.security && this.application.config.sessions.persist) {
    if (this.session && (this.sessionCreated || this.sessionModified)) {
      application.security.savePersistentSession(this, this.sid, callback);
    } else callback();
  } else callback();
};
