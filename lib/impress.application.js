...
impress.application.mixin = (application) => {
  ...
  function checkTemplate(tplName, tplPath) {
    api.fs.exists(tplPath, (exists) => {
      if (exists) application.systemTemplates[tplName] = tplPath;
    });
  }
  ...
  application.prepareScript = (
    ...
    try {
      if (source[0] === impress.ASCII_BRACE_OPENING) {
        source = '(' + source + ')';
      }
      const code = impress.USE_STRICT + source;
      const options = {
        filename: fileName,
      ...
      const res = script.runInContext(application.sandbox, options);
      let exports = application.sandbox.module.exports;
      if (!exports) exports = res;
      application.sandbox.module = {};
  ...
  application.preloadDirectory = (
    ...
    if (!callback) callback = api.common.emptiness;
  ...
  application.loadApi = (callback) => {
    ...
    api.fs.readdir(absPath, (err, files) => {
      ...
      api.metasync.each(files, (fileName, cb) => {
        ...
        api.fs.stat(filePath, (err, stats) => {
          if (err || !stats.isDirectory()) return cb();
    ...
    api.fs.readdir(logDir, (err, files) => {
      if (err) {
        ...
        api.metasync.filter(files, (file, cb) => {
          ...
          if (!impress.mode) return cb(!fileName.includes('.'));
          ...
          api.metasync.series(files, application.loadConfigFile, () => {
            ...
            impress.nodeId = (application === impress) ?
      ...
      if (lst && !minified) {
        ...
        if (pre) {
          ...
          if (data) {
            ...
            if (callback) {
              const error = new Error(ext + ' parse/preprocess error');
              callback(error);
            }
