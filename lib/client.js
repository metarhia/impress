...
Client.prototype.dispatch = function() {
  ...
  this.parseCookies();
  const code = impress.firewall.check(this);
...
Client.prototype.block = function(
  ...
  impress.firewall.ip.deny(this.ipInt, msec);
  if (this.sid) impress.firewall.sid.deny(this.sid, msec);
};
...
Client.prototype.error = function(
  ...
  this.fileHandler('error', false, () => {
    ...
    application.log.error(`HTTP ${code}\t${message}\t${this.url}`);
  });
};
