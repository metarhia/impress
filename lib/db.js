...
db.openApplicationDatabases = function(application, callback) {
  if (application.config.databases) {
    ...
    api.async.each(Object.keys(databases), function(alias, cb) {
      var database = databases[alias];
      db.openDatabase(application, alias, database, cb);
    }, callback);
  } else callback();
};
...
//   alias - global accessible database name
//   database - database definition (see /config/databases.js)
//   callback - function to be called when done
//
db.openDatabase = function(application, alias, database, callback) {
