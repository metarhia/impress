...
impress.log.mixin = (application) => {
  ...
  application.log.init = () => {
    ...
    const makeTimer = (fileType) => (
      () => application.log.flush(fileType, api.common.emptiness)
    );
    ...
    application.log.open = (callback) => {
      if (!application.isImpress && !applicationLog) return callback();
      ...
      api.fs.readdir(application.log.dir, (err, files) => {
        ...
        for (i in files) {
          ...
          if (fileAge > 1 && fileAge > keepDays) {
            api.fs.unlink(
              application.log.dir + '/' + files[i], api.common.emptiness
            );
          }
        }
      });
    ...
    application.log.flush = (fileType, callback) => {
      const file = application.log.files.get(fileType);
      if (!file || file.lock || file.buf.length === 0) return callback();
      file.lock = true;
      const buf = file.buf;
      file.buf = '';
      file.fd.write(buf, () => {
    ...
    api.metasync.each(application.log.fileTypes, (fileType /*cb*/) => {
      ...
      application.log[fileType] = (
        application.isImpress ? (serverLog ? fnI : fnN) :
        (
          serverLog ?
          (applicationLog ? fnAI : fnI) :
          (applicationLog ? fnA : fnN)
        )
      );
    });
