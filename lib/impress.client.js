...
var Client = function(application, req, res) {
	...
	this.pathDir = application.hostDir+this.path;
...
Client.prototype.processing = function() {
	...
	this.detectRealPath(function() {
		...
		else if (client.typeExt !== 'ws' && client.res.setHeader && !client.res.headersSent) {
									...
									api.async.parallel({
										...
										if (results.stats) {
											...
											if (results.stats.isDirectory()) {
												...
												// Read metadata
												if (ext === 'json') {
													var exports, parName, parameter, verb;
	...
	if (fileCache) {
		...
		if (exists) {
			...
			if (client.execPath !== '/' && file !== 'meta') {
				...
				client.execPathDir = application.hostDir+client.execPath;
...
Client.prototype.detectRealPath = function(callback) {
	...
	if ((folderCache && folderCache !== impress.fileExists) || client.realPath === '/') callback();
		...
		if (exists) {
			...
			client.realPathDir = application.hostDir+client.realPath;
				...
				if (sinceTime && impress.isTimeEqual(sinceTime, stats.mtime)) client.error(304);
					...
					if (stats.isDirectory()) client.index(filePath); else {
						if (stats.size < application.config.files.cacheMaxFileSize) {
							impress.compress(filePath, stats, application, client, httpCode);
						} else {
							client.res.writeHead(200, impress.baseHeader(client.ext, stats));
