	...
	if (impress.security.hasDb(application)) {
		rl.question("Delete stored sessions and users if any? [y/n]: ", function(answer) {
			...
			if (answer === 'y') {
				...
				api.async.series(steps, function(err, results) {
		...
		if (!user) {
			if (impress.security.hasDb(client.application)) {
				client.application.databases.security.users.insert({
					...
					password: client.fields.Password ? client.fields.Password : impress.security.generateKey(),
					group: "users"
				}, function(err, users) {
					if (!err && users.length>0) {
						var user = impress.security.user(users[0]);
						...
						if (user.group) session.group = user.group;
