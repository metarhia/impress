...
impress.mixinApplication = function(application) {
  ...
  // Dispatch requests
  //
  if (application !== impress) application.dispatcher = function(req, res) {
    var staticRx, route, match, urlRoute, form,
    ...
    if (application.config.routes) {
      for (var iRoute = 0; iRoute < application.config.routes.length; iRoute++) {
        ...
        if (match) return application.dispatchRoute(client, route, match, req, res);
      }
    }
    if (staticRx && staticRx.test(client.url)) client.static();
      ...
      } else client.dispatcher();
    }
  };
  
  // Dispatch route
  //
  application.dispatchRoute = function(client, route, match, req, res) {
    ...
    else if (inArray(req.usedRoutes, iRoute)) client.error(508);
    else {
      req.url = urlRoute;
