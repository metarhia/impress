...
function loadPlugins() {
  if (impress.config.plugins) {
    ...
    for (i = 0; i < impress.config.plugins.length; i++) {
...
impress.server.start = function() {
  impress.mixinApplication(impress, function() {
    ...
    function startup() {
      ...
      if (typeof(global.gc) === 'function' && impress.config.cluster.gcInterval > 0) {
        setInterval(function() {
          global.gc();
        }, impress.config.cluster.gcInterval);
      }
      isFirstStart = false;
    }
  });
...
function startIpc() {
  ...
  if (api.cluster.isMaster) {
    if (impress.config.cluster.strategy === 'multiple' || impress.config.cluster.strategy === 'sticky') {
      var workerId;
      for (workerId = 0; workerId < impress.config.cluster.workers; workerId++) {
    ...
    process.on('message', function(message, socket) {
      ...
      if (message.name === 'impress:socket') {
        ...
        if (application) application.workers[message.wId] = message;
      } else if (message.name === 'impress:exitlongworker') {
        if (application) delete application.workers[message.wId+''];
      }
    });
  }
}
...
impress.forkLongWorker = function(appName, workerFile, clientData) {
  ...
  if (application) {
    ...
    impress.stat.forkCount++;
    worker.on('exit', function(code, signal) {
      impress.retranslateEvent(-1, { name:'impress:exitlongworker', appName: appName, wId: worker.id });
      impress.stat.forkCount--;
...
//   workerFile - filename with path
//   workerId   - kill worker by id
//
impress.killLongWorker = function(appName, workerFile, workerId) {
  var application = impress.applications[appName];
  if (application) {
    var workerId, worker;
    for (workerId in application.longWorkers) {
      worker = application.longWorkers[workerId];
      if (worker.file === workerFile && (!workerId || (worker.id === workerId))) {
        worker.kill();
        impress.stat.forkCount++;
      }
    }
...
impress.listenWorker = function(worker) {
  worker.on('message', function(message) {
    if (message.name === 'impress:event') { // propagate to all workers except of original sender
      ...
      message.wId = impress.forkLongWorker(message.appName, message.workerFile, message.clientData);
      impress.retranslateEvent(-1 /*worker*/, message);
