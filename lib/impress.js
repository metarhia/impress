...
function loadPlugins() {
  var plugins = impress.dataByPath(impress, 'config.sandbox.plugins');
  if (plugins) {
    ...
    impress.loadConfig(function() {
      ...
      impress.log.open(function() {
        if (impress.workerId === 'long') {
          impress.nodeId = impress.config.scale.cluster+'L'+process.pid;
...
impress.forkLongWorker = function(appName, workerFile, clientData) {
  ...
  if (application) {
    ...
    worker.nodeId = impress.config.scale.cluster+'L'+worker.process.pid;
    impress.listenWorker(worker);
...
impress.listenWorker = function(worker) {
  worker.on('message', function(message) {
    if (message.name === 'impress:event') { // propagate to all workers except of original sender
      ...
      if (api.cluster.isMaster && impress.config.scale && (impress.config.scale.instance === 'server')) {
