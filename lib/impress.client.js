...
const extCases = {
  ...
  sse: (client, application) => { // eslint-disable-line
    if (!application.sse) return client.error(510);
    client.sseConnect();
  },

  ws: (client, application) => {
    if (!application.websocket) return client.error(510);
  ...
  jsonp: (client, application) => { // eslint-disable-line
    if (!client.context.data) return client.error(400);
  ...
  csv: (client, application) => { // eslint-disable-line
    if (!client.context.data) return client.error(400);
