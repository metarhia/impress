...
impress.log.mixinApplication = function (application) {
  ...
  var writeInterval;
  if (application.config.log && application.config.log.writeInterval) writeInterval = application.config.log.writeInterval;
  else if (impress.config.log && impress.config.log.writeInterval) writeInterval = impress.config.log.writeInterval;
    ...
    if (application === impress || applicationLog) {
      api.mkdirp(application.log.dir, function(err) {
        if (err) console.error(err);
          ...
          nextReopen = nextDate - now + dayMilliseconds;
  ...
  application.log.write = function(fileType, message) {
    ...
    if (file) {
      ...
      file.buf += msg;
      if (impress.config.log && impress.config.log.stdout && inArray(impress.config.log.stdout, fileType)) {
        if (fileType === 'error') msg = msg.replace(/;/g, '\n ').replace(/\t/g, '\n').bold.red;
        else if (fileType === 'debug') msg = msg.bold.green;
        console.log(msg);
      }
    }
  };
